...
def draw_ellipse0(array_bgr, row, col, r_radius, c_radius, color_brg, alpha_transp=0):
    ...
    if alpha_transp > 0:
        res_rgb[
            ellipse(
                int(row), int(col), int(r_radius), int(c_radius), shape=array_bgr.shape
            )
        ] = array_bgr[
            ellipse(
                int(row), int(col), int(r_radius), int(c_radius), shape=array_bgr.shape
            )
        ] * alpha_transp + color_brg * (
            1 - alpha_transp
        )
    else:
        res_rgb[
...
def draw_circle(array_bgr, row, col, rad, color_brg, alpha_transp=0):
    ...
    if alpha_transp > 0:
        res_rgb[
            circle(int(row), int(col), int(rad), shape=array_bgr.shape)
        ] = array_bgr[
            circle(int(row), int(col), int(rad), shape=array_bgr.shape)
        ] * alpha_transp + color_brg * (
            1 - alpha_transp
...
def draw_line(array_bgr, row1, col1, row2, col2, color_bgr, alpha_transp=0.0):
    ...
    for i in range(0, rr.shape[0]):
        if (
            rr[i] >= 0
            and rr[i] < array_bgr.shape[0]
            and cc[i] >= 0
            and cc[i] < array_bgr.shape[1]
        ):
...
def draw_rect(
    image, col_left, row_up, col_right, row_down, color, w=2, alpha_transp=0.8
):

    lines = numpy.array(
        (
            (col_left, row_up, col_left, row_down),
            (col_left, row_down, col_right, row_down),
            (col_right, row_down, col_right, row_up),
            (col_right, row_up, col_left, row_up),
        )
    )
    points_2d = numpy.array(
        (
            (col_left, row_up),
            (col_left, row_down),
            (col_right, row_down),
...
def draw_rects(image, rects, color, w=2, alpha_transp=0.8):
    for r in rects:
        image = draw_rect(
            image, r[0, 0], r[0, 1], r[1, 0], r[1, 1], color, w, alpha_transp
        )
...
def draw_contours(
    ...
    if color_fill is not None:
        clr_fill = (
            ...
            color_fill[1],
            color_fill[2],
            int(255 - int(255 - transp_fill * 255)),
        )
    else:
        clr_fill = None
    ...
    if color_outline is not None:
        clr_outline = (
            ...
            color_outline[1],
            color_outline[2],
            int(255 - int(255 - transp_outline * 255)),
        )
    else:
        clr_outline = None
...
def draw_convex_hull_cv(image, points, color=(255, 255, 255), transperency=0.0):
    ...
    cntrs = (
        numpy.array(points, dtype=numpy.int)[hull.vertices]
        .reshape(1, -1, 1, 2)
        .astype(numpy.int)
    )
...
def draw_convex_hull(image, points, color=(255, 0, 0), transperency=0.0):
    ...
    if points[:, 0].max() - points[:, 0].min() < 1:
        return image
    ...
    try:
        ...
        draw.polygon(
            polypoints, (color[0], color[1], color[2], int(255 - transperency * 255))
        )
        result = numpy.array(pImage)
        del draw
    except:
...
def draw_ellipse(image, p, color=(255, 0, 0), transperency=0.0):
    ...
    if len((image.shape)) == 3:
        ...
        draw.ellipse(
            ...
            fill=(color[0], color[1], color[2], int(255 - transperency * 255)),
            outline=(color[0], color[1], color[2], 255),
        )
    else:
        draw = ImageDraw.Draw(pImage)
        draw.ellipse(
            (int(p[0]), int(p[1]), int(p[2]), int(p[3])), fill=color, outline=color
        )
...
def get_colors(N, shuffle=False, colormap="jet", alpha_blend=None):
    ...
    if colormap == "rainbow":
        ...
        colors = [
            cv2.cvtColor(
                numpy.array([c, 255, 255], dtype=numpy.uint8).reshape((1, 1, 3)),
                cv2.COLOR_HSV2BGR,
            )[0, 0]
            for c in colors
        ]
        ...
        colors = numpy.array(
            [
                (int(255 * i / (N - 1)), int(255 * i / (N - 1)), int(255 * i / (N - 1)))
                for i in range(N)
            ]
        )
    ...
    if alpha_blend is not None:
        colors = [
            (
                (alpha_blend) * numpy.array((255, 255, 255))
                + (1 - alpha_blend) * numpy.array(color)
            )
            for color in colors
        ]
...
# ----------------------------------------------------------------------------------------------------------------------
def draw_ellipses(
    image, ellipses, color=(255, 255, 255), w=4, draw_axes=False, labels=None
):
...
def draw_lines(image, lines, color=(255, 255, 255), w=4, put_text=False):
    ...
    for id, line in enumerate(lines):
        if line is None:
            continue
        (x1, y1, x2, y2) = line
        if numpy.any(numpy.isnan((x1, y1, x2, y2))):
            continue
    ...
    for id, point in enumerate(points):
        ...
        if put_text:
            cv2.putText(
                result,
                "{0}".format(id),
                (min(W - 10, max(10, x1)), min(H - 5, max(10, y1))),
                cv2.FONT_HERSHEY_SIMPLEX,
                1 * w / 12,
                clr,
                1,
...
def draw_signals_v2(signals, lanes, colors, w=3):
    ...
    if lanes is not None:
        for p in lanes:
            if p is not None:
                cv2.line(
                    image_signal, (p, 0), (p, height), color=(64, 64, 64), thickness=1
                )
...
# ----------------------------------------------------------------------------------------------------------------------
def draw_cuboid(
    image, points_2d, lines_idx=None, color=(255, 255, 255), w=1, put_text=False
):
...
def extend_view_from_image(image, factor=4, color_bg=(32, 32, 32)):
    ...
    image_small = cv2.resize(
        tools_image.saturate(image), (int(W / factor), int(H / factor))
    )
    image_result = tools_image.put_image(
        image_result, image_small, int((H - H / factor) / 2), int((W - W / factor) / 2)
    )
