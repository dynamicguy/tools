...
class Hough(object):
    ...
    def get_lines_cv(self, skeleton, the_range=None, min_weight=50, max_count=10):
        ...
        candidates = cv2.HoughLines(
            skeleton,
            10,
            (numpy.pi / 180) / 20,
            threshold=0,
            min_theta=the_range[0],
            max_theta=the_range[-1],
        )
        ...
        for line in candidates:
            if len(lines) < max_count:
                ...
                x2 = int(x0 + L * b)
                y2 = int(y0 - L * a)
                if self.line_exists((x1, y1, x2, y2), lines):
                    continue
    ...
    def get_lines_ski(self, skeleton, the_range, min_weight, max_count=10):
        ...
        idx_all = numpy.dstack(
            numpy.unravel_index(
                numpy.argsort(-hspace.ravel()), (hspace.shape[0], hspace.shape[1])
            )
        )[0]
        ...
        for n, idx in enumerate(idx_all):
            ...
            if w >= min_weight and len(lines) < max_count:
                ...
                if self.is_boarder_line(
                    (x1, y1, x2, y2), skeleton.shape[0], skeleton.shape[1]
                ):
                    continue
                if self.line_exists((x1, y1, x2, y2), lines):
                    continue
        ...
        lines, weights = self.sort_lines_and_weights(
            numpy.array(lines), numpy.array(weights)
        )
    ...
    def get_lines_ski_segments(self, image, the_range, min_weight, max_count=10):
        ...
        candidates = probabilistic_hough_line(
            gray, threshold=10, line_length=50, line_gap=10, theta=the_range
        )
        lines, weights = [], []
    ...
    def line_exists(self, candidate, lines):
        ...
        tol = 10
        for line in lines:
            d = tools_render_CV.distance_between_lines(
                candidate,
                line,
                clampA0=True,
                clampA1=True,
