...
class tools_TS(object):
    ...
    def load_and_normalize(self, filename_input):
        ...
        dataset = tools_IO.load_mat_pd(
            filename_input, delim=self.delim, dtype=numpy.float32
        )[: self.max_len]
    ...
    def E2E_train_test(
        ...
        suffix = ""
        if target_column >= 0:
            suffix = "%03d_" % target_column
        filename_train_fact = path_output + "train_fact" + suffix + ".txt"
        filename_test_fact = path_output + "test_fact" + suffix + ".txt"
        filename_train_pred = (
            path_output + "train_pred_" + suffix + self.classifier.name + ".txt"
        )
        filename_test_pred = (
            path_output + "test_pred_" + suffix + self.classifier.name + ".txt"
        )
        ...
        if target_column >= 0:
            ...
            tools_IO.save_mat(
                self.denormalize(dataset[:train_size]), filename_train_fact
            )
            tools_IO.save_mat(
                self.denormalize(dataset[train_size:]), filename_test_fact
            )
            tools_IO.save_mat(
                self.denormalize(numpy.array(Y_train_predicts).T), filename_train_pred
            )
            tools_IO.save_mat(
                self.denormalize(numpy.array(Y_test_predicts).T), filename_test_pred
            )

        tools_plot.plot_two_series(
            filename_train_fact,
            filename_train_pred,
            caption="train",
            filename_out=path_output + self.classifier.name + "_train.png",
        )
        tools_plot.plot_two_series(
            filename_test_fact,
            filename_test_pred,
            caption="test",
            filename_out=path_output + self.classifier.name + "_test.png",
        )
            ...
            for col in range(0, dataset.shape[1]):
                Y_predict.append(
                    self.classifier.train(
                        numpy.delete(dataset, col, axis=1), dataset[:, col]
                    )
                )
