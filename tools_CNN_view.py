...
def colorize_chess(image, W, H):
    ...
    if len(image.shape) == 2:
        ...
        image_color = image
    r, c = numpy.meshgrid(
        numpy.linspace(0, H - 1, num=H), numpy.linspace(0, W - 1, num=W)
    )
    ...
    for R in numpy.arange(0, image.shape[0], H):
        for C in numpy.arange(0, image.shape[1], W):
            if (int(R / H) + int(C / W)) % 2 == 0:
                ...
                red, green, blue = 1.05, 1.05, 0.95
            image_color[R + r, C + c, 0] = numpy.clip(
                image_color[R + r, C + c, 0] * blue, 0, 255
            )
            image_color[R + r, C + c, 1] = numpy.clip(
                image_color[R + r, C + c, 1] * green, 0, 255
            )
            image_color[R + r, C + c, 2] = numpy.clip(
                image_color[R + r, C + c, 2] * red, 0, 255
            )
...
def tensor_gray_1D_to_image(tensor, orientation="landscape"):
    ...
    if orientation == "landscape":
        image = numpy.reshape(
            tensor, (numpy.minimum(rows, cols), numpy.maximum(rows, cols))
        )
    else:
        image = numpy.reshape(
            tensor, (numpy.maximum(rows, cols), numpy.minimum(rows, cols))
        )
...
# ----------------------------------------------------------------------------------------------------------------------
def tensor_gray_3D_to_image(
    tensor, do_colorize=False, do_scale=False, force_rows_dim2=None
):
...
# ---------------------------------------------------------------------------------------------------------------------
def tensor_color_4D_to_image(
    tensor, do_chess=False, force_rows_dim3=None, force_rows_dim2=None
):
...
# ---------------------------------------------------------------------------------------------------------------------
def tensor_gray_4D_to_image(
    tensor,
    do_colorize=False,
    do_scale=False,
    force_rows_dim3=None,
...
def visualize_filters(keras_model, path_out):
    ...
    for i in range(0, len(keras_model.layers)):
        ...
        if isinstance(tensor, Conv2D):
            ...
            if tensor.shape[2] == 3:
                cv2.imwrite(
                    path_out + "filter_%03d.png" % i, tensor_color_4D_to_image(tensor)
                )
            else:
                cv2.imwrite(
                    path_out + "filter_%03d.png" % i,
                    tensor_gray_4D_to_image(tensor, do_colorize=True),
                )
...
def stage_tensors(outputs, path_out, names):
    ...
    for i in range(0, len(outputs)):
        ...
        if len(tensor.shape) == 3:
            if tensor.shape[2] == 3:
                cv2.imwrite(path_out + "layer_%s.png" % (names[i]), tensor)
            elif tensor.shape[2] != 3:
                cv2.imwrite(
                    path_out + "layer_%s.png" % (names[i]),
                    tensor_gray_3D_to_image(tensor, do_colorize=False),
                )
        elif len(tensor.shape) == 1:
            cv2.imwrite(
                path_out + "layer_%s.png" % (names[i]),
                tools_image.hitmap2d_to_viridis(tensor_gray_1D_to_image(tensor)),
            )
...
def visualize_layers(keras_model, filename_input, path_out, need_scale=False):
    ...
    layer_names = [
        "%02d_%s" % (i, layer.name)
        for i, layer in zip(range(len(keras_model.layers)), keras_model.layers)
    ][1:]
    outputs = Model(inputs=keras_model.input, outputs=layer_outputs).predict(
        numpy.array([image])
    )
...
def add_de_pool_layer(invmodel, orig_layer_after, input_shape=None):
    scale = (
        orig_layer_after.input.get_shape().as_list()[1]
        / orig_layer_after.output.get_shape().as_list()[1]
    )
    scale = int(scale + 0.5)
...
def add_de_conv_layer(invmodel, orig_conv_layer, crop=None, input_shape=None):
    ...
    if input_shape is None:
        invmodel.add(
            Conv2DTranspose(
                N,
                orig_conv_layer.kernel_size,
                activation=orig_conv_layer.activation,
                padding=padding,
...
def construct_weights_2x2x3(n=16 * 3):
    ...
    for c, i in zip([0, 1, 0], [0, 16, 16 * 2]):
        F[:, :, c, i + 0] = (
            numpy.array([[-1, -1], [-1, -1]]).astype(numpy.float32) / 4.0
        )
        F[:, :, c, i + 1] = (
            numpy.array([[-1, -1], [-1, +1]]).astype(numpy.float32) / 4.0
        )
...
def construct_filters_2x2(n_filters=16):
    ...
    return [
        construct_weights_2x2()[:, :, :, :n_filters],
        numpy.zeros(16, numpy.float32)[:n_filters],
    ]
