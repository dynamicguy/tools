...
def debug_projection(X_source, X_target, result):
    ...
    xmin = (
        int(min(X_source[:, 0].min(), X_target[:, 0].min(), result[:, 0].min()))
        - padding
    )
    xmax = (
        int(max(X_source[:, 0].max(), X_target[:, 0].max(), result[:, 0].max()))
        + padding
    )
    ymin = (
        int(min(X_source[:, 1].min(), X_target[:, 1].min(), result[:, 1].min()))
        - padding
    )
    ymax = (
        int(max(X_source[:, 1].max(), X_target[:, 1].max(), result[:, 1].max()))
        + padding
    )
    ...
    for i, (x, y) in enumerate(X_target):
        cv2.circle(
            ...
            18,
            (int(colors_t[i][0]), int(colors_t[i][1]), int(colors_t[i][2])),
            2,
        )
    for i, (x, y) in enumerate(result):
        cv2.circle(
            ...
            (int(x - xmin), int(y - ymin)),
            12,
            (int(colors_s[i][0]), int(colors_s[i][1]), int(colors_s[i][2])),
            2,
        )
...
def fit_affine(X_source, X_target, do_debug=False):
    A, _ = cv2.estimateAffine2D(
        numpy.array(X_source), numpy.array(X_target), confidence=0.95
    )
...
def fit_pnp(landmarks_3d, landmarks_2d, mat_camera_3x3, dist=numpy.zeros(5)):
    ...
    landmarks_2d_check, jac = cv2.projectPoints(
        landmarks_3d, r_vec, t_vec, mat_camera_3x3, dist
    )
...
def fit_manual(X3D, target_2d, fx, fy, xref=None, lref=None, do_debug=True):
    ...
    for a0 in rotation_range:
        ...
        result_E_ref = cv2.transform(projection_2d_ref.reshape(-1, 1, 2), E).reshape(
            (-1, 2)
        )
        result_E_lin = cv2.transform(projection_2d_lin.reshape(-1, 1, 2), E).reshape(
            (-1, 2)
        )
        ...
        if do_debug:
            ...
            for x in result_E_ref:
                cv2.circle(image, (int(x[0]), int(x[1])), 4, (128, 128, 128), -1)
            for i in numpy.arange(0, result_E_lin.shape[0] - 1, 2):
                cv2.line(
                    image,
                    (int(result_E_lin[i, 0]), int(result_E_lin[i, 1])),
                    (int(result_E_lin[i + 1, 0]), int(result_E_lin[i + 1, 1])),
                    (128, 128, 128),
                    2,
                )
...
def fit_R(points_3d, points_2d, camera_matrix, rvec, tvec):
    ...
    for a_pitch_deg in range_pitch:
        for a_yaw_deg in range_yaw:
            r = numpy.array(rvec).flatten() + numpy.array(
                (
                    a_pitch_deg * numpy.pi / 180,
                    a_yaw_deg * numpy.pi / 180,
                    a_roll_deg * numpy.pi / 180,
                )
            )
            points_2d_cand, err = project_points(
                points_3d, r, tvec_new, camera_matrix, numpy.zeros(5)
            )
    ...
    points_2d_cand, err = project_points(
        points_3d, g_r, tvec_new, camera_matrix, numpy.zeros(5)
    )
    # image_result = tools_draw_numpy.draw_points(empty, points_2d_cand, color=(0, 128, 255), w=16)
    # image_result = tools_draw_numpy.draw_points(image_result, points_2d, color=(0, 0, 255), w=8)
...
def compose_projection_mat_3x3(fx, fy, fov_x=0.5, fov_y=0.5):
    f = 0.5 * fx / (fov_x)
    mat_camera = numpy.array([[f, 0.0, fx / 2], [0.0, f, fy / 2], [0.0, 0.0, 1.0]])
    return mat_camera


# ----------------------------------------------------------------------------------------------------------------------
def compose_projection_mat_4x4(fx, fy, fov_x=0.5, fov_y=0.5):
    mat_camera = numpy.array(
        [[fx, 0, fx * fov_x, 0], [0, fy, fy * fov_y, 0], [0, 0, 1, 0], [0, 0, 0, 1]]
    )
    return mat_camera
...
def compose_projection_ortho_mat(fx, fy, scale_factor):
    P = numpy.array(
        [[fx, 0, 0, fx / 2], [0, fy, 0, fy / 2], [0, 0, 1, 0], [0, 0, 0, 1]]
    )
    P[:, 3] *= scale_factor[0]
    P /= scale_factor[0]
...
def ETU_to_mat_view(eye2, target2, up2):
    ...
    dot2 = -numpy.array(
        (numpy.dot(side2, eye2), numpy.dot(up2, eye2), numpy.dot(forward2, eye2))
    )
...
def euler_to_quaternion(rvec):
    ...
    qx = numpy.sin(roll / 2) * numpy.cos(pitch / 2) * numpy.cos(yaw / 2) - numpy.cos(
        roll / 2
    ) * numpy.sin(pitch / 2) * numpy.sin(yaw / 2)
    qy = numpy.cos(roll / 2) * numpy.sin(pitch / 2) * numpy.cos(yaw / 2) + numpy.sin(
        roll / 2
    ) * numpy.cos(pitch / 2) * numpy.sin(yaw / 2)
    qz = numpy.cos(roll / 2) * numpy.cos(pitch / 2) * numpy.sin(yaw / 2) - numpy.sin(
        roll / 2
    ) * numpy.sin(pitch / 2) * numpy.cos(yaw / 2)
    qw = numpy.cos(roll / 2) * numpy.cos(pitch / 2) * numpy.cos(yaw / 2) + numpy.sin(
        roll / 2
    ) * numpy.sin(pitch / 2) * numpy.sin(yaw / 2)
...
def apply_RT(rvec, tvec, X):
    ...
    R = pyrr.matrix44.create_from_matrix33(
        cv2.Rodrigues(numpy.array(rvec, dtype=numpy.float))[0]
    )
    T = pyrr.matrix44.create_from_translation(tvec).T
    M = pyrr.matrix44.multiply(T, R)
...
def RT_to_H(RT, camera_matrix_3x3, Z):
    ...
    P = compose_projection_mat_4x4(
        camera_matrix_3x3[0, 0],
        camera_matrix_3x3[1, 1],
...
def project_points_MVP(points_3d, img, mat_projection, mat_view, mat_model, mat_trns):
    ...
    camera_matrix_3x3 = compose_projection_mat_3x3(
...
# ----------------------------------------------------------------------------------------------------------------------
def project_points_MVP_GL(
    points_3d, img, mat_projection, mat_view, mat_model, mat_trns
):
...
def reverce_project_points_Z0_M(points_2d, PM):
    ...
    for (i, j) in points_2d:
        A = numpy.array(
            (
                ((PM[0, 0] - i * PM[2, 0]), (PM[0, 1] - i * PM[2, 1])),
                ((PM[1, 0] - j * PM[2, 0]), (PM[1, 1] - j * PM[2, 1])),
            )
        )
...
# ----------------------------------------------------------------------------------------------------------------------
def project_points_M(
    points_3d, RT, camera_matrix_3x3, dist=numpy.zeros(5), do_flip=False
):
...
def derive_dims(cuboid_3d):
    ...
    h = numpy.abs(
        cuboid_3d[[1, 3, 5, 7], idx_h].mean() - cuboid_3d[[0, 2, 4, 6], idx_h].mean()
    )
    ps = cuboid_3d[[1, 3, 5, 7], :]
    ps = ps[:, idx_lw]

    d = numpy.array(
        [numpy.linalg.norm(ps[i] - ps[j]) for i, j in zip([0, 1, 2, 3], [1, 2, 3, 0])]
    )
...
def fit_1M(cuboid):
    ...
    cube_1M = numpy.array(
        [
            ...
            [i - w / 2 for i in [0, 0, w, w, w, w, 0, 0]],
        ],
        dtype=numpy.float32,
    ).T
...
def fit_3D_3D(cuboid, idx_h=1, idx_wh=[0, 2], do_debug=False):
    ...
    rect_h0_reg = tools_render_CV.regularize_rect(
        rect_h0, inlined=False, do_debug=do_debug
    )
    rect_h1_reg = tools_render_CV.regularize_rect(
        rect_h1, inlined=False, do_debug=do_debug
    )
    cuboid2 = cuboid.copy()
