...
class CNN_Inception_TF(object):
    def __init__(self):
        ...
        with gfile.FastGFile(
            self.dest_directory + "classify_image_graph_def.pb", "rb"
        ) as f:
            self.graph_def = tf.GraphDef()
            self.graph_def.ParseFromString(f.read())
            (
                self.tensor_bottleneck,
                self.tensor_jpeg_data,
                self.tensor_resized_input,
                self.tensor_classification,
            ) = tf.import_graph_def(
                self.graph_def,
                name="",
                return_elements=[
    ...
    def maybe_download_and_extract(self):
        ...
        if not os.path.exists(filepath):
            ...
            print()
            print(
                "Successfully downloaded", filename, os.stat(filepath).st_size, "bytes."
            )
            ...
            if not os.path.isfile(feature_filename):
                ...
                for b, local_filename in enumerate(local_filenames):
                    ...
                    image_data = gfile.FastGFile(
                        path_input + each + "/" + local_filename, "rb"
                    ).read()
                    if image_data is None:
                        continue
                    feature = sess.run(
                        self.tensor_bottleneck, {self.tensor_jpeg_data: image_data}
                    )[0]
    ...
    def predict_classes(self, path_input, filename_output, limit=1000000, mask="*.png"):
        ...
        for each in patterns:
            ...
            for i in range(0, local_filenames.shape[0]):
                image_data = gfile.FastGFile(
                    path_input + each + "/" + local_filenames[i], "rb"
                ).read()
                prob = sess.run(
                    self.tensor_classification, {self.tensor_jpeg_data: image_data}
                )[0][:1000]
                idx = numpy.argsort(-prob)[0]
                label = self.class_names[idx]
                tools_IO.save_labels(
                    path_input + each + "/" + filename_output,
