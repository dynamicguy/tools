...
class Face_filter(object):
    ...
    def restore_model(self, filename_G_weights, filename_D_weights):
        self.G.load_state_dict(
            torch.load(filename_G_weights, map_location=lambda storage, loc: storage)
        )
        self.D.load_state_dict(
            torch.load(filename_D_weights, map_location=lambda storage, loc: storage)
        )
        return
        ...
        with torch.no_grad():
            ...
            for i, (x_real, c_org) in enumerate(self.data_loader):
                ...
                c_trg_list = self.create_labels(
                    c_org, c_dim=self.c_dim, selected_attrs=self.selected_attrs
                )
    ...
    def process_file(self, filename_in, filename_out):
        ...
        image_result = numpy.zeros(
            (self.image_size * 2, self.image_size * 4, 3), dtype=numpy.uint8
        )
        ...
        for i, c_trg in enumerate(c_trg_list):
            ...
            if i == 0:
                image_result = tools_image.put_image(
                    image_result, newlook, 0, self.image_size * 2 + 0
                )
            if i == 1:
                image_result = tools_image.put_image(
                    image_result, newlook, 0, self.image_size * 2 + self.image_size
                )
            if i == 2:
                image_result = tools_image.put_image(
                    image_result, newlook, self.image_size, self.image_size * 2 + 0
                )
            if i == 4:
                image_result = tools_image.put_image(
                    image_result,
    ...
    # ----------------------------------------------------------------------------------------------------------------------
    def process_folder(
        self, path_input, path_out, list_of_masks="*.png,*.jpg", limit=1000000
    ):
    ...
    def train(self, image_dir, attr_path):
        ...
        dataset = CelebA(
            image_dir, attr_path, self.selected_attrs, self.transform, "test"
        )
