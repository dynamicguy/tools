...
class detector_landmarks(object):
    def __init__(self, filename_config, filename_3dmarkers=None):
        ...
        if filename_3dmarkers is not None:
            self.model_68_points = tools_IO.load_mat(
                filename_3dmarkers, dtype=numpy.float, delim=","
            )
    ...
    def draw_landmarks_v2(
        self, image, landmarks, del_triangles=None, color=(0, 128, 255), w=6
    ):
    ...
    # ----------------------------------------------------------------------------------------------------------------------
    def cut_face(
        self, image, L, desiredLeftEye=(0.35, 0.35), target_W=256, target_H=256
    ):
    ...
    # --------------------------------------------------------------------------------------------------------------------
    def get_pose_perspective(
        self, image, landmarks_2d, landmarks_3d, mat_trns=None, idx_match=None
    ):
        ...
        if landmarks_3d is None:
            L3D = self.model_68_points
        else:
            L3D = landmarks_3d.copy()
    ...
    # --------------------------------------------------------------------------------------------------------------------
    def get_pose_ortho(
        self, image, landmarks_2d, landmarks_3d, mat_trns=None, do_debug=False
    ):
        ...
        rvec, tvec, scale_factor, modelview = tools_render_CV.find_ortho_fit(
            L3D, landmarks_2d, fx, fy, do_debug=do_debug
        )
        loss = tools_render_CV.check_projection_ortho_modelview(
            L3D, landmarks_2d, modelview, fx, fy, scale_factor, do_debug=True
        )
    ...
    def draw_annotation_box(
        self, image, rotation_vector, translation_vector, color=(0, 128, 255), w=1
    ):

        point_3d = []

        if self.model_68_points.max() - self.model_68_points.min() > 5:
            rear_size, rear_depth, front_size, front_depth = (
                numpy.array([75, 0, 100, 100]) * 0.50
            )
        else:
            rear_size, rear_depth, front_size, front_depth = (
                numpy.array([55, 0, 75, 55]) * 0.015
            )
        ...
        (point_2d, _) = cv2.projectPoints(
            point_3d, rotation_vector, translation_vector, camera_matrix, dist_coefs
        )
    ...
    def draw_annotation_box_v2(self, image, rvec, tvec, scale):
        ...
        points_2d, _ = tools_pr_geom.project_points_ortho(
            points_3d,
            rvec.flatten(),
            tvec.flatten(),
            numpy.zeros(4),
