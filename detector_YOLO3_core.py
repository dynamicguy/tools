...
# ----------------------------------------------------------------------------------------------------------------------
def get_tensors_box_score_class(
    yolo_outputs,
    anchors,
    anchor_mask,
    num_classes,
...
# ----------------------------------------------------------------------------------------------------------------------
def get_tensors_box_score_class_unfilter(
    yolo_outputs,
    anchors,
    anchor_mask,
    num_classes,
...
def preprocess_true_boxes(true_boxes, input_shape, anchors, anchor_mask, num_classes):
    ...
    for b in range(m):
        ...
        intersect_wh = numpy.maximum(intersect_maxes - intersect_mins, 0.0)
        ...
        for t, n in enumerate(best_anchor):
            for l in range(num_layers):
                if n in anchor_mask[l]:
                    i = numpy.floor(true_boxes[b, t, 0] * grid_shapes[l][1]).astype(
                        "int32"
                    )
                    j = numpy.floor(true_boxes[b, t, 1] * grid_shapes[l][0]).astype(
                        "int32"
                    )
...
def get_targets(list_of_boxes, input_shape, anchors, anchor_mask, num_classes):
    ...
    for k, boxes in enumerate(list_of_boxes):
        ...
        y = preprocess_true_boxes(
            true_boxes, input_shape, anchors, anchor_mask, num_classes
        )
...
# ----------------------------------------------------------------------------------------------------------------------
def save_targets(
    folder_out, list_of_boxes, input_shape, anchors, anchor_mask, num_classes
):

    store0 = tools_HDF5.HDF5_store(
        filename=folder_out + "target_0.hdf5",
        object_shape=(13, 13, 3, 5 + num_classes),
        dtype=numpy.float32,
    )
    store1 = tools_HDF5.HDF5_store(
        filename=folder_out + "target_1.hdf5",
        object_shape=(26, 26, 3, 5 + num_classes),
        dtype=numpy.float32,
    )
    if len(anchor_mask) > 2:
        store2 = tools_HDF5.HDF5_store(
            filename=folder_out + "target_2.hdf5",
            object_shape=(52, 52, 3, 5 + num_classes),
            dtype=numpy.float32,
        )
...
# ----------------------------------------------------------------------------------------------------------------------
def yolo_loss(
    args, anchors, anchor_mask, num_classes, ignore_thresh=0.5, print_loss=False
):
...
# ----------------------------------------------------------------------------------------------------------------------
def get_true_boxes(
    foldername, filename, delim="\t", smart_resized_target=None, limit=100
):
...
def draw_annotation_boxes(file_annotations, path_out, colors, class_names, delim=" "):
    ...
    for filename in set(filenames):
        ...
        for box, class_ID in zip(box_xyxy[idx], class_IDs[idx]):
            cv2.rectangle(
                image, (box[0], box[1]), (box[2], box[3]), colors[class_ID], thickness=2
            )
