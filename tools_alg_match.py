...
def calc_hit_field(image_base, image_pattern):
    ...
    hitmap2 = calc_hit_field_basic(image_base, image_pattern[x1:x2, y1:y2]).astype(
        numpy.float
    )
    hitmap3 = calc_hit_field_basic(image_base, image_pattern[x1:x2, :]).astype(
        numpy.float
    )
    hitmap4 = calc_hit_field_basic(image_base, image_pattern[:, y1:y2]).astype(
        numpy.float
    )
...
def calc_hit_field_basic(image_base, image_pattern, rotation_tol=0, rotation_step=0):
    ...
    hitmap_2d = tools_image.canvas_extrapolate_gray(
        hitmap_gray, image_base.shape[0], image_base.shape[1]
    )

    if rotation_tol != 0 and rotation_step != 0:
        for angle in numpy.arange(-rotation_tol, +rotation_tol, rotation_step):
            M = cv2.getRotationMatrix2D(
                (image_pattern.shape[1] / 2, image_pattern.shape[1] / 2), angle, 1
            )
            image_pattern_rotated = cv2.warpAffine(
                image_pattern,
                M,
                (image_pattern.shape[1], image_pattern.shape[0]),
                borderMode=cv2.BORDER_REPLICATE,
            )
            hitmap_gray = cv2.matchTemplate(
                image_base, image_pattern_rotated, method=method
            )
            min = hitmap_gray.min()
            max = hitmap_gray.max()
            hitmap_gray = (225 * (hitmap_gray - min) / (max - min)).astype(numpy.uint8)
            hitmap_gray = tools_image.canvas_extrapolate_gray(
                hitmap_gray, image_base.shape[0], image_base.shape[1]
            )
...
def visualize_matches_map(disp_col, disp_v1, disp_v2, disp_h1, disp_h2):
    image_disp_hor = numpy.zeros((disp_col.shape[0], disp_col.shape[1])).astype(
        numpy.float32
    )
...
def get_best_matches(
    image1, image2, disp_v1, disp_v2, disp_h1, disp_h2, window_size=15, step=10
):

    N = int(image1.shape[0] * image1.shape[1] / step / step)
    rand_points = numpy.random.rand(N, 2)
    rand_points[:, 0] = window_size + (
        rand_points[:, 0] * (image1.shape[0] - 2 * window_size)
    )
    rand_points[:, 1] = window_size + (
        rand_points[:, 1] * (image1.shape[1] - 2 * window_size)
    )
    ...
    for each in rand_points:
        ...
        template = tools_image.crop_image(
            image1,
            row - window_size,
            col - window_size,
            row + window_size,
        ...
        top, left, bottom, right = (
            row - window_size + disp_v1,
            col - window_size + disp_h1,
            row + window_size + disp_v2,
            col + window_size + disp_h2,
        )
        field = tools_image.crop_image(image2, top, left, bottom, right)

        if (
            numpy.min(template[:, :, 0]) == numpy.max(template[:, :, 0])
            and numpy.min(template[:, :, 1]) == numpy.max(template[:, :, 1])
            and numpy.min(template[:, :, 2]) == numpy.max(template[:, :, 2])
        ):
        ...
        if q_best > 0:
            ...
            if (
                col + dc >= 0
                and col + dc < image1.shape[1]
                and row + dr >= 0
                and row + dr < image1.shape[0]
            ):
...
def get_corners_Harris(image1):
    ...
    corners = cv2.cornerSubPix(
        gray, numpy.float32(centroids), (5, 5), (-1, -1), criteria
    ).astype(int)
...
def get_disparity_BM(imgL, imgR, disp_v1, disp_v2, disp_h1, disp_h2):
    ...
    dispr = numpy.flip(
        stereo.compute(numpy.flip(imgR, axis=1), numpy.flip(imgL, axis=1)), axis=1
    )
    return -displ / 16, -dispr / 16


# ---------------------------------------------------------------------------------------------------------------------
def get_disparity_from_matches(
    rows, cols, coord1, coord2, disp_v1, disp_v2, disp_h1, disp_h2
):
...
# ---------------------------------------------------------------------------------------------------------------------
def get_matches_from_desc_limit_by_disp(
    points_source,
    des_source,
    points_destin,
    des_destin,
    ...
    for i in range(0, len(points_source)):
        ...
        for j in range(0, len(points_destin)):
            ...
            if (
                (col2 - col1 >= disp_h1)
                and (col2 - col1 < disp_h2)
                and (row2 - row1 >= disp_v1)
                and (row2 - row1 < disp_v2)
            ):
                idx.append(j)
        ...
        if idx.size != 0:
            ...
            match1, match2, dist = get_matches_from_keypoints_desc(
                pnts_src, des_src, pnts_dest, des_dest, matchtype
            )
            if match1.size != 0:
...
# ---------------------------------------------------------------------------------------------------------------------
def get_matches_from_keypoints_desc(
    ...
    if matchtype == "knn":
        ...
        if des_destin.shape[0] > des_source.shape[0]:
            zzz = numpy.zeros(
                (+des_destin.shape[0] - des_source.shape[0], des_destin.shape[1])
            )
        ...
        matches = get_matches_on_desc(
            des_source.astype(numpy.uint8), des_destin.astype(numpy.uint8)
        )
    ...
    if len(matches) == 0:
        ...
        for m in matches:
            if (
                m.queryIdx < points_destin.shape[0]
                and m.trainIdx < points_source.shape[0]
            ):
...
def get_matches_on_desc_flann(des_source, des_destin):
    ...
    matches = flann.knnMatch(
        des_destin.astype(numpy.float32), des_source.astype(numpy.float32), k=2
    )
...
def reproject_matches(
    ...
    R = window_size
    canv1 = tools_image.canvas_extrapolate(
        image1, image1.shape[0] + 2 * R, image1.shape[1] + 2 * R
    )
    canv2 = tools_image.canvas_extrapolate(
        image2, image2.shape[0] + 2 * R, image2.shape[1] + 2 * R
    )
    coord1 = crd1.copy()
    coord2 = crd2.copy()
    ...
    for i in range(0, coord1.shape[0]):
        small = canv1[
            coord1[i, 1] - R : coord1[i, 1] + R, coord1[i, 0] - R : coord1[i, 0] + R
        ]
        acc1_color[
            coord2[i, 1] - R : coord2[i, 1] + R, coord2[i, 0] - R : coord2[i, 0] + R
        ] += small
        acc1_count[
            coord2[i, 1] - R : coord2[i, 1] + R, coord2[i, 0] - R : coord2[i, 0] + R
        ] += 1
