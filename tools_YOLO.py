...
def generate_colors(N=80):
    ...
    for i in range(0, N):
        ...
        color = cv2.cvtColor(
            numpy.array([hue, 255, 225], dtype=numpy.uint8).reshape(1, 1, 3),
            cv2.COLOR_HSV2BGR,
        )[0][0]
...
def draw_classes_on_image(image, boxes_yxyx, scores, color, draw_score=False):
    ...
    for box, score in zip(boxes_yxyx, scores):
        ...
        image = tools_draw_numpy.draw_rect(
            image, left, top, right, bottom, color, alpha_transp=1.0 - score
        )
        if draw_score == True:
...
# ----------------------------------------------------------------------------------------------------------------------
def get_true_boxes(
    foldername, filename, smart_resized_target=None, delim=" ", limit=1000000
):
...
def get_images(foldername, filename, delim=" ", smart_resized_target=None, limit=10000):
    ...
    for filename in filenames_dict:
        ...
        if smart_resized_target is not None:
            image = tools_image.smart_resize(
                image, smart_resized_target[0], smart_resized_target[1]
            )
...
def draw_annotation_boxes(
    file_annotations, file_classes, file_metadata, path_out, delim=" "
):
    ...
    with open(file_annotations) as f:
        lines = f.readlines()[1:]
    boxes_xyxy = numpy.array(
        [line.split(delim)[1:5] for line in lines], dtype=numpy.int
    )
    ...
    for filename in set(filenames):
        ...
        statuses = are_boxes_preprocessed_well(
            boxes_resized, anchors, anchor_mask, len(class_names)
        )
        ...
        for box, class_ID, status in zip(boxes_xyxy[idx], class_IDs[idx], statuses):
            ...
            cv2.rectangle(
                image, (box[0], box[1]), (box[2], box[3]), colors[class_ID], thickness=w
            )
            cv2.putText(
                image,
                "{0:d} {1:s}".format(class_ID, class_names[class_ID]),
                (box[0], box[1] - 4),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.6,
                colors[class_ID],
...
def are_boxes_preprocessed_well(boxes, anchors, anchor_mask, num_classes):
    ...
    for box in boxes:
        ...
        y = detector_YOLO3_core.preprocess_true_boxes(
            true_boxes, (416, 416), anchors, anchor_mask, num_classes
        )
...
def draw_and_save(
    ...
    if filename_out is not None:
        res_image = draw_objects_on_image(
            tools_image.desaturate(image, 1.0),
            boxes_yxyx,
            scores,
            classes,
            colors,
...
def save_metadata(
    ...
    for i, each in enumerate(anchor_mask):
        ET.SubElement(xml_anchor_mask, "M" + str(i)).text = str(
...
def load_metadata(filename_input):
    ...
    for each in root.findall("obj_threshold"):
        obj_threshold = float(each.text)
    for each in root.findall("nms_threshold"):
        nms_threshold = float(each.text)
    for each in root.findall("input_image_size"):
        input_image_size = (
            int(each.text.split(",")[0].split("(")[-1]),
            int(each.text.split(",")[1].split(")")[0]),
        )
    for each in root.findall("anchors")[0]:
        anchors.append(
            (
                float(each.text.split(",")[0].split("(")[-1]),
                float(each.text.split(",")[1].split(")")[0]),
            )
        )
    ...
    for each in root.findall("class_names")[0]:
        class_names.append(each.text)
...
def plot_cluster_result(clusters, nearest_clusters, wh, k):
    ...
    for icluster in numpy.unique(nearest_clusters):
        ...
        plt.plot(
            ...
            "o",
            alpha=0.5,
            label="cluster = {}, N = {:6.0f}".format(icluster, numpy.sum(pick)),
        )
        plt.xlabel("width")
        plt.ylabel("height")
    plt.legend()
