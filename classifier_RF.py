...
class classifier_RF(object):
    ...
    # ----------------------------------------------------------------------------------------------------------------------
    def learn_file_batch(
        self, filename_train, has_header=True, n_epochs=1, batch_size=20, verbose=False
    ):
        self.model = RandomForestClassifier(
            warm_start=True, n_estimators=1, max_depth=self.max_depth
        )
        ...
        for e in range(n_epochs):
            for b, split in enumerate(splits):
                if verbose:
                    print("Batch %03d/%d" % (b, len(splits)))
                data = numpy.array(
                    tools_IO.get_lines(
                        filename_train,
                        start=has_header * 1 + split[0],
                        end=has_header * 1 + split[-1],
                    )
                )
