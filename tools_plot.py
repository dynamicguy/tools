...
# ----------------------------------------------------------------------------------------------------------------------
def plot_hystogram(
    plt, H, label=None, SMAPE=None, xmin=None, xmax=None, ymax=None, filename_out=None
):
...
def plot_series(S, labels=None, SMAPE=None, filename_out=None):
    ...
    if labels is not None:
        patches = [
            mpatches.Patch(color=colors_list[i], label=labels[i])
            for i in reversed(range(0, S.shape[1]))
        ]
...
def plot_two_series(filename1, filename2, caption="", filename_out=None):
    ...
    if len(mat1.shape) == 1:
        ...
        for i in range(0, S):
            ...
            plot_series(
                numpy.vstack((mat1[:, i], mat2[:, i])).T, labels=labels, SMAPE=SMAPE
            )
        plt.tight_layout()
...
def plot_multiple_series(
    filename_fact, list_filenames, target_column=0, caption="", filename_out=None
):
    ...
    Labels_train = numpy.array(
        ["fact"] + [each.split("/")[-1] for each in list_filenames]
    )
...
def plot_2D_samples_from_folder(foldername, caption="", add_noice=0):
    ...
    for filename in local_filenames:
        data = tools_IO.load_mat(
            foldername + filename, dtype=numpy.chararray, delim="\t"
        )
        labels = (data[:, 0]).astype("float32")
        ...
        plt.plot(
            X[:, 0], X[:, 1], "ro", color=colors_list[i % len(colors_list)], alpha=0.4
        )
        i += 1
...
def plot_2D_features_multi_Y(plt, X, Y, labels=None, filename_out=None):
    ...
    for i, each in enumerate(numpy.unique(Y)):
        ...
        if labels is not None:
            patches.append(
                mpatches.Patch(color=colors_list[i % len(colors_list)], label=labels[i])
            )
...
def plot_2D_features_pos_neg(X, Y, filename_out=None):
    ...
    norm = (
        max(
            tools_IO.max_element_by_value(dict_pos)[1],
            tools_IO.max_element_by_value(dict_neg)[1],
        )
        / max_size
    )
...
def plot_2D_scores(
    ...
    if not os.path.isfile(filename_data_pos):
        return
    if not os.path.isfile(filename_data_neg):
        return
    if not os.path.isfile(filename_data_grid):
        return
    ...
    xx, yy = numpy.meshgrid(
        numpy.linspace(minx, maxx, num=S), numpy.linspace(miny, maxy, num=S)
    )

    plt.contourf(xx, yy, numpy.flip(grid_scores, 0), cmap=cm.coolwarm, alpha=0.8)
...
# ----------------------------------------------------------------------------------------------------------------------
def display_roc_curve_from_descriptions(
    plt,
    figure,
    filename_scores_pos,
    filename_scores_neg,
    ...
    labels = numpy.hstack(
        (numpy.full(len(scores_pos), 1), numpy.full(len(scores_neg), 0))
    )
...
def display_distributions(
    plt, fig, path_scores1, path_scores2, delim=" ", inverse_score=0, filename_out=None
):
    scores1 = []
    scores2 = []
...
def plot_confusion_mat(plt, fig, filename_mat, caption=""):
    ...
    confusion_mat = tools_IO.load_mat(filename_mat, dtype=numpy.chararray, delim="\t")[
        :, :2
    ]
    patterns = numpy.unique(confusion_mat[:, 0])

    labels_fact = numpy.array(
        [tools_IO.smart_index(patterns, each) for each in confusion_mat[:, 0]]
    )
    labels_pred = numpy.array(
        [tools_IO.smart_index(patterns, each) for each in confusion_mat[:, 1]]
    )

    mat, descriptions, sorted_labels = tools_IO.preditions_to_mat(
        labels_fact, labels_pred, numpy.unique(confusion_mat[:, 0])
    )
...
def plot_histo(dict_H, filename_out=None, colors=None):
    ...
    if colors is not None and len(colors) == len(dict_H):
        for i in range(len(colors)):
            barlist[i].set_color(
                (colors[i, 2] / 255, colors[i, 1] / 255, colors[i, 0] / 255)
            )
    plt.xticks(xticks)
...
def pairplot(X, Y, folder_out):
    ...
    for i in range(X.shape[1]):
        plt.clf()
        plot_1D_features_pos_neg(
            plt,
            X[:, i],
            Y,
            labels=Y,
            filename_out=folder_out + "plt_%02d_%02d.png" % (i, i),
        )
