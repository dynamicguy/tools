...
class render_GL(object):
    ...
    def __initTexture(self, inputimage):
        ...
        gluBuild2DMipmaps(
            GL_TEXTURE_2D, 3, width, height, GL_RGBA, GL_UNSIGNED_BYTE, image
        )
        return texture
    ...
    def __draw_2d_mesh(self, H, W, points_coord, points_text_coord, triangles):
        ...
        for triangle in triangles:
            ...
            glTexCoord2f(
                float(tx0 / texture_width), texture_height - float(ty0 / texture_height)
            )
            glVertex2f(x0, y0)

            glTexCoord2f(
                float(tx1 / texture_width), texture_height - float(ty1 / texture_height)
            )
            glVertex2f(x1, y1)

            glTexCoord2f(
                float(tx2 / texture_width), texture_height - float(ty2 / texture_height)
            )
            glVertex2f(x2, y2)
    ...
    def morph_2D_mesh(self, H, W, points_coord, points_text_coord, triangles):
        ...
        image_buffer = glReadPixels(
            0, 0, W, H, OpenGL.GL.GL_RGB, OpenGL.GL.GL_UNSIGNED_BYTE
        )
