...
class VBO(object):
    def __init__(self):
        ...
        self.data = numpy.full(
            self.texture_offset + 3 * 2 * self.n_faces, 0.0, dtype="float32"
        )
    ...
    # ----------------------------------------------------------------------------------------------------------------------
    def append_object(
        self, filename_obj, do_normalize_model_file, svec=(1, 1, 1), tvec=(0, 0, 0)
    ):
        ...
        for idxv, idxn, idxt in zip(
            object.idx_vertex, object.idx_normal, object.idx_texture
        ):
            ...
            self.data[self.iterator : self.iterator + 9] = (
                object.coord_vert[idxv]
            ).flatten()
            self.data[
                self.iterator
                + self.color_offset : self.iterator
                + self.color_offset
                + 9
            ] = clr
            self.data[
                self.iterator
                + self.normal_offset : self.iterator
                + self.normal_offset
                + 9
            ] = (object.coord_norm[idxn]).flatten()
            self.data[
                2 * self.iterator // 3
                + self.texture_offset : 2 * self.iterator // 3
                + self.texture_offset
                + 6
...
class render_GL3D(object):
    def __init__(
        ...
        self.object = self.my_VBO.append_object(
            filename_obj, self.do_normalize_model_file, tvec=tvec
        )
        self.update_texture()
    ...
    def update_texture(self):
        ...
        if (self.object.filename_texture[-1] is None) or (
            not os.path.exists(self.object.filename_texture[-1])
        ):
        ...
        glTexImage2D(
            ...
            0,
            GL_RGB,
            texture_image.shape[1],
            texture_image.shape[0],
            0,
            GL_RGB,
            GL_UNSIGNED_BYTE,
            texture_image.flatten(),
        )
    ...
    def bind_VBO(self, wired_mode=False):
        ...
        glBufferData(
            GL_ARRAY_BUFFER,
            item_size * len(self.my_VBO.data),
            self.my_VBO.data,
            GL_STATIC_DRAW,
        )
        ...
        glVertexAttribPointer(
            0, 3, GL_FLOAT, GL_FALSE, item_size * 3, ctypes.c_void_p(0)
        )
    ...
    def __init_mat_projection_perspective(self, a_fov_x=0.5, a_fov_y=0.5):
        self.mat_projection = tools_pr_geom.compose_projection_mat_4x4_GL(
            self.W, self.H, a_fov_x, a_fov_y
        )
        glUniformMatrix4fv(
    ...
    def __init_mat_projection_ortho(self, scale_factor=(5, 5)):
        ...
        self.mat_projection = pyrr.matrix44.create_orthogonal_projection(
    ...
    def init_mat_view_direct(self, mat_view):
        ...
        glUniformMatrix4fv(
            glGetUniformLocation(self.shader, "view"), 1, GL_FALSE, self.mat_view
        )
        return

    # ----------------------------------------------------------------------------------------------------------------------
    def init_mat_view_ETU(self, eye, target, up):
        self.mat_view = tools_pr_geom.ETU_to_mat_view(
            numpy.array(eye), numpy.array(target), numpy.array(up)
        )
        glUniformMatrix4fv(
            glGetUniformLocation(self.shader, "view"), 1, GL_FALSE, self.mat_view
        )
    ...
    def init_mat_view_RT(self, rvec, tvec, do_rodriges=False, do_flip=True):
        self.mat_view = tools_pr_geom.compose_RT_mat(
            rvec, tvec, do_rodriges=do_rodriges, do_flip=do_flip
        )
        glUniformMatrix4fv(
            glGetUniformLocation(self.shader, "view"), 1, GL_FALSE, self.mat_view
        )
    ...
    def init_mat_model_direct(self, mat_model):
        ...
        glUniformMatrix4fv(
            glGetUniformLocation(self.shader, "model"), 1, GL_FALSE, self.mat_model
        )
        return

    # ----------------------------------------------------------------------------------------------------------------------
    def init_mat_model(self, rvec, tvec, do_rodriges=False, do_flip=False):
        self.mat_model = tools_pr_geom.compose_RT_mat(
            rvec, tvec, do_rodriges=do_rodriges, do_flip=do_flip
        )
        glUniformMatrix4fv(
            glGetUniformLocation(self.shader, "model"), 1, GL_FALSE, self.mat_model
    ...
    def __init_mat_transform(self, scale_vec):
        ...
        glUniformMatrix4fv(
            glGetUniformLocation(self.shader, "transform"), 1, GL_FALSE, self.mat_trns
        )
        return
    ...
    def __init_mat_light(self, r_vec):
        ...
        glUniformMatrix4fv(
            glGetUniformLocation(self.shader, "light"), 1, GL_FALSE, self.mat_light
        )
        return
    ...
    def get_image(
        ...
        if mat_proj is not None:
            self.mat_projection = mat_proj
        if mat_view is not None:
            self.mat_view = mat_view
        if mat_model is not None:
            self.mat_model = mat_model
        if mat_trans is not None:
            self.mat_trns = mat_trans
        ...
        image_buffer = glReadPixels(
            0, 0, self.W, self.H, OpenGL.GL.GL_RGB, OpenGL.GL.GL_UNSIGNED_BYTE
        )
        image = numpy.frombuffer(image_buffer, dtype=numpy.uint8).reshape(
            self.H, self.W, 3
        )
    ...
    # ----------------------------------------------------------------------------------------------------------------------
    def init_perspective_view_1_mat_model(
        self, rvec, tvec, a_fov_x=0.5, a_fov_y=0.5, scale=(1, 1, 1)
    ):
        # OK
        tvec = numpy.array(tvec, dtype=float)
        self.init_mat_view_RT(
            numpy.array(rvec, dtype=float), tvec, do_rodriges=True, do_flip=True
        )
    ...
    # ----------------------------------------------------------------------------------------------------------------------
    def init_perspective_view_1_mat_view(
        self, rvec, tvec, a_fov_x=0.5, a_fov_y=0.5, scale=(1, 1, 1)
    ):
        tvec = numpy.array(tvec, dtype=float)
        self.init_mat_model(
    ...
    def draw_debug_info(self, image):
        ...
        if self.projection_type == "P":
            ...
            result = tools_render_CV.draw_points_numpy_MVP_GL(
                self.object.coord_vert,
                image,
                self.mat_projection,
                self.mat_view,
                self.mat_model,
            ...
            result = tools_render_CV.draw_points_numpy_MVP_ortho(
                self.object.coord_vert,
                result,
                self.mat_projection,
                self.mat_view,
        ...
        self.draw_vec(
            numpy.array((Y, P, R)) * 180 / numpy.pi, 20, 420, result, "Model rot deg"
        )
    ...
    def stage_data(self, folder_out):
        ...
        cv2.imwrite(
            folder_out + "screenshot_%03d.png" % i, self.get_image(do_debug=True)
        )
    ...
    def load_markers(self, filename_in, filename_marker_obj, marker_scale=None):
        ...
        for marker in markers:
            self.my_VBO.append_object(
                filename_marker_obj,
                do_normalize_model_file=True,
                svec=(self.marker_scale, self.marker_scale, self.marker_scale),
                tvec=marker,
            )
        self.bind_VBO()
        return
    ...
    def translate_view(self, delta_translate):
        ...
        TT = pyrr.matrix44.multiply(
            pyrr.matrix44.inverse(self.mat_view),
            pyrr.matrix44.multiply(T, self.mat_view),
        )
    ...
    def rotate_view(self, delta_angle):
        ...
        R = pyrr.matrix44.create_from_eulers(
            numpy.array((delta_angle[0], delta_angle[1], delta_angle[2]))
        )
        RR = pyrr.matrix44.multiply(
            pyrr.matrix44.multiply(self.mat_view, R),
            pyrr.matrix44.inverse(self.mat_view),
        )
    ...
    def rotate_model(self, delta_angle, silent=False):
        ...
        if self.on_rotate and self.mat_model_rotation_checkpoint is not None:
            S, Q, tvec = pyrr.matrix44.decompose(
                self.mat_model_rotation_checkpoint.copy()
            )
        else:
    ...
    def translate_model(self, delta_pos):
        ...
        if self.on_translate and self.mat_model_translation_checkpoint is not None:
            rvec, tvec = tools_pr_geom.decompose_to_rvec_tvec(
                self.mat_model_translation_checkpoint.copy()
            )
        else:
