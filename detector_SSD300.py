...
class detector_SSD300(object):
    def __init__(self, model_weights_h5, filename_metadata=None, obj_threshold=None):
        ...
        K.clear_session()
        self.model = detector_SSD300_core.ssd_300(
            image_size=(self.input_image_size[0], self.input_image_size[1], 3),
    ...
    def process_image(self, image):
        ...
        t_predictions = tf.convert_to_tensor(predictions, dtype=tf.float32)

        decoded_predictions = detector_SSD300_core.DecodeDetections(
            confidence_thresh=0.01,
        ...
        y_pred_thresh = numpy.array(
            [y_pred[k][y_pred[k, :, 1] > 0.5] for k in range(y_pred.shape[0])]
        )[0]
        classes = numpy.array(y_pred_thresh[:, 0], dtype=numpy.int)
        scores = y_pred_thresh[:, 1]
        boxes_yxyx = numpy.array(
            [
                y_pred_thresh[:, 3],
                y_pred_thresh[:, 2],
                y_pred_thresh[:, 5],
    ...
    # ----------------------------------------------------------------------------------------------------------------------
    def process_folder(
        self, path_input, path_out, mask="*.jpg", limit=1000000, markup_only=False
    ):
    ...
    def process_annotation(
        self, file_annotations, path_out, folder_annotation=None, markup_only=False
    ):
        ...
        filename_markup_out = (
            file_annotations.split("/")[-1].split(".")[0] + "_pred.txt"
        )
        ...
        for local_filename in local_filenames:
            ...
            for each in self.process_file(
                foldername + local_filename, filename_image_out
            ):
                result.append(each)
    ...
    # ----------------------------------------------------------------------------------------------------------------------
    def save_default_model_and_metadata(
        self,
    ...
    # ----------------------------------------------------------------------------------------------------------------------
    def learn(self, file_annotations, folder_out, folder_annotation=None, limit=4000):
        ...
        true_boxes = tools_SSD.get_true_boxes(file_annotations, delim=" ", limit=limit)
        images = tools_SSD.get_images(
            foldername, file_annotations, delim=" ", resized_target=(300, 300)
        )
        ...
        ssd_input_encoder = detector_SSD300_core.SSDInputEncoder(
            img_height=self.input_image_size[0],
            img_width=self.input_image_size[1],
            n_classes=len(self.class_names) - 1,
        ...
        targets = ssd_input_encoder(true_boxes)
        self.model.fit(
            x=[images], y=[targets], validation_split=0.3, verbose=2, epochs=100
        )
