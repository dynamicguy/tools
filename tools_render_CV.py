...
def draw_axis(img, camera_matrix, dist, rvec, tvec, axis_length):
    ...
    axis_2d_start, jac = cv2.projectPoints(
        axis_3d_start, rvec, tvec, camera_matrix, dist
    )
    ...
    img = tools_draw_numpy.draw_line(
        img,
        axis_2d_start[0, 1],
        axis_2d_start[0, 0],
        axis_2d_end[0, 1],
...
def draw_grid(image, camera_matrix, dist, rvec, tvec, color=(0, 0, 190), w=1):
    ...
    for x in numpy.arange(x_min, x_max, step):
        ...
        image_result = tools_draw_numpy.draw_lines(
            image_result, lines_2d.reshape((-1, 4)), color, w
        )
...
# ----------------------------------------------------------------------------------------------------------------------
def draw_compass_p3x4(
    image, p3x4, R, Z=0, step=1, color=(0, 128, 255), draw_labels=False
):
    points_3d = numpy.array(
        [
            (
                R * numpy.sin(angle * numpy.pi / 180.0),
                R * numpy.cos(angle * numpy.pi / 180.0),
                Z,
            )
    ...
    if numpy.array(1 * idx).sum() > 1:
        ...
        lines_2d = [
            (
                points_2d_visible[p, 0],
                points_2d_visible[p, 1],
                points_2d_visible[p + 1, 0],
                points_2d_visible[p + 1, 1],
            )
        ...
        if draw_labels:
            ...
            image_result = tools_draw_numpy.draw_points(
                image_result, points_2d[idx], colors[idx], labels=labels[idx]
            )
    ...
    if camera_matrix.shape[0] == 3 and camera_matrix.shape[1] == 3:
        ...
        M = pyrr.matrix44.multiply(
            pyrr.matrix44.create_from_translation(
                numpy.array(tvec, dtype=float).flatten()
            ).T,
            pyrr.matrix44.create_from_matrix33(
                cv2.Rodrigues(numpy.array(rvec, dtype=float).flatten())[0]
            ),
        )
        ...
        points_2dx = tools_pr_geom.project_points_p3x4(
            points_3d, camera_matrix, check_reverce=True
        )
...
# ----------------------------------------------------------------------------------------------------------------------
def draw_image(
    im_target, im_source_one, camera_matrix, dist, rvec, tvec, scale=(1, 1, 1)
):

    H, W = im_source_one.shape[:2]
    points_2d_source = numpy.array(
        [[0, 0], [0, H], [W, H], [W, 0]], dtype=numpy.float32
    )
    ...
    points_2d_target, jac = tools_pr_geom.project_points(
        points_3d, rvec, tvec, camera_matrix, dist
    )

    homography, result = tools_pr_geom.fit_homography(
        points_2d_source, points_2d_target
    )

    image_trans = cv2.warpPerspective(
        im_source_one, homography, (im_target.shape[1], im_target.shape[0])
    )
    mask = cv2.warpPerspective(
        numpy.ones((H, W)), homography, (im_target.shape[1], im_target.shape[0])
    )
    mask = ~(mask > 0)
...
def draw_cube_numpy(
    ...
    if method == "cv":
        points_2d, jac = cv2.projectPoints(
            ...
            numpy.array(tvec, dtype=float).reshape((3, 1)),
            camera_matrix,
            dist,
        )
    else:
        points_2d, jac = tools_pr_geom.project_points(
            points_3d, rvec, tvec, camera_matrix, dist
        )
...
def draw_cube_numpy_MVP_GL(
    ...
    img = draw_lines_numpy_MVP(
        numpy.array(lines_3d),
        img,
        mat_projection,
        mat_view,
        mat_model,
...
def draw_points_numpy_MVP_GL(
    ...
    camera_matrix_3x3 = tools_pr_geom.compose_projection_mat_3x3(
        img.shape[1], img.shape[0], 1 / mat_projection[0][0], 1 / mat_projection[1][1]
    )

    # M = pyrr.matrix44.multiply(mat_view, pyrr.matrix44.multiply(mat_model, mat_trns))
    # L3D = tools_pr_geom.apply_matrix(M.T, points_3d)

    M0 = pyrr.matrix44.multiply(
        mat_view.T, pyrr.matrix44.multiply(mat_model.T, mat_trns.T)
    )
    ...
    if method == 0:
        ...
        for point in points_2d:
            ...
            if numpy.any(numpy.isinf(point)):
                continue
            img = tools_draw_numpy.draw_circle(
                img, int(point[1]), int(point[0]), w, color
            )
...
# ----------------------------------------------------------------------------------------------------------------------
def draw_points_numpy_RT(
    points_3d, img, RT, camera_matrix_3x3, color=(66, 0, 166), w=6
):
    points_2d = tools_pr_geom.project_points_M(
        points_3d, RT, camera_matrix_3x3, numpy.zeros(5)
    ).reshape((-1, 2))
...
def draw_lines_numpy_RT(lines_3d, img, RT, camera_matrix_3x3, color=(66, 0, 166), w=6):
    points_2d_start = tools_pr_geom.project_points_M(
        lines_3d[:, :3], RT, camera_matrix_3x3, numpy.zeros(5)
    ).reshape((-1, 2))
    points_2d_end = tools_pr_geom.project_points_M(
        lines_3d[:, 3:], RT, camera_matrix_3x3, numpy.zeros(5)
    ).reshape((-1, 2))
...
def draw_lines_numpy_MVP(
    ...
    points_2d_start = tools_pr_geom.project_points_MVP(
        lines_3d[:, :3], img, mat_projection, mat_view, mat_model, mat_trns
    )
    points_2d_end = tools_pr_geom.project_points_MVP(
        lines_3d[:, 3:], img, mat_projection, mat_view, mat_model, mat_trns
    )

    for point_start, point_end in zip(points_2d_start, points_2d_end):
        if numpy.any(numpy.isnan(point_start)) or numpy.any(numpy.isnan(point_end)):
            continue
        if numpy.any(numpy.isinf(point_start)) or numpy.any(numpy.isinf(point_end)):
            continue
        cv2.line(
            img,
            (int(point_start[0]), int(point_start[1])),
...
# ----------------------------------------------------------------------------------------------------------------------
def draw_points_numpy_MVP_ortho(
    points_3d,
    img,
    mat_projection,
    mat_view,
    ...
    points_2d, jac = tools_pr_geom.project_points_ortho(
        X4D[:, :3], (0, 0, 0), (0, 0, 0), numpy.zeros(4), fx, fy, scale_factor
    )
    points_2d = points_2d.reshape((-1, 2))
...
# ----------------------------------------------------------------------------------------------------------------------
def check_projection_ortho(
    L3D, points_2d, rvec, tvec, fx, fy, scale_factor, do_debug=False
):

    L3D_ortho = L3D.copy()

    points_2d_check, _ = tools_pr_geom.project_points_ortho(
        L3D_ortho, rvec.flatten(), tvec.flatten(), numpy.zeros(4), fx, fy, scale_factor
    )
    ...
    if do_debug:
        ...
        for x in points_2d:
            cv2.circle(image, (int(x[0]), int(x[1])), 4, (0, 128, 255), -1)
        for x in points_2d_check:
            cv2.circle(image, (int(x[0]), int(x[1])), 4, (0, 32, 190), -1)
        cv2.imwrite("./images/output/fit_check_RT.png", image)
    return loss


# ----------------------------------------------------------------------------------------------------------------------
def check_projection_ortho_modelview(
    L3D, points_2d, modelview, fx, fy, scale_factor, do_debug=False
):

    points_2d_check, _ = tools_pr_geom.project_points_ortho_modelview(
        L3D, modelview, numpy.zeros(4), fx, fy, scale_factor
    )
    points_2d_check = points_2d_check.reshape((-1, 2))
    loss = math.sqrt(((points_2d_check - points_2d) ** 2).mean())
    if do_debug:
...
def line_intersection_sympi_slow(line1, line2):
    res = intersection(
        Line(Point(line1[0], line1[1]), Point(line1[2], line1[3])),
        Line(Point(line2[0], line2[1]), Point(line2[2], line2[3])),
    )
...
def get_interception_ray_triangle(pos, direction, triangle):
    ...
    collision = line_plane_intersection(
        n, triangle[0, :3], direction[:3], pos[:3], epsilon=1e-6
    )
...
def fit_scale_factor(X4D, X_target, a0, a1, a2, fx, fy, do_debug=False):
    ...
    if do_debug:
        ...
        for x in X_target:
            cv2.circle(image, (int(x[0]), int(x[1])), 16, (0, 128, 255), 4)
        for x in result_E:
            cv2.circle(image, (int(x[0]), int(x[1])), 4, (0, 32, 190), -1)
        # for x in X_projection  : cv2.circle(image, (int(x[0]), int(x[1])), 3, (128, 128, 128), -1)

        cv2.imwrite(
            "./data/output/fit_%03d_%03d.png"
            % (a0 * 180 / math.pi, a1 * 180 / math.pi),
            image,
        )
...
def find_ortho_fit(X3D, X_target, fx, fy, do_debug=False):
    ...
    for a0 in rotation_range:
        for a1 in rotation_range:
            ...
            scale_factor, TR, result = fit_scale_factor(
                X4D, X_target, a0, a1, a2, fx, fy, do_debug
            )
...
def distance_between_lines(
    ...
    if (
        line1 is None
        or numpy.any(numpy.isnan(line1))
        or line2 is None
        or numpy.any(numpy.isnan(line2))
    ):
...
def distance_extract_to_line(extract, line, do_debug=False):
    ...
    if do_debug:
        ...
        cv2.line(
            image,
            (int(extract[0]), int(extract[1])),
            (int(extract[2]), int(extract[3])),
            color=color_blue,
        )
...
def line_box_intersection(line, box):
    ...
    for segm in segms:
        if numpy.all(line[:2] == segm[:2]):
            ...
            p1, p2, d = distance_between_lines(
                segm, line, clampA0=True, clampA1=True, clampB0=False, clampB1=False
            )
            if numpy.abs(d) < tol:
...
def circle_line_intersection(
    center, R, pt1, pt2, full_line=True, tangent_tol=1e-9, do_debug=False
):
...
def ellipse_line_intersection(ellipse, line, full_line=True, do_debug=False):
    ...
    M = numpy.array(
        [
            [numpy.cos(-rotation_angle_rad), -numpy.sin(-rotation_angle_rad), 0],
            [numpy.sin(-rotation_angle_rad), numpy.cos(-rotation_angle_rad), 0],
            [0, 0, 1],
        ]
    )
    ...
    intersections_trans = circle_line_intersection(
        (0, 0), Rxy[0], pt1.flatten(), pt2.flatten(), full_line=full_line
    )
    if len(intersections_trans) > 0 and Rxy[1] > 0:
        intersections = numpy.array(intersections_trans).copy()
        intersections[:, 1] /= Rxy[0] / Rxy[1]
        intersections = cv2.transform(
            numpy.array(intersections, dtype=numpy.float32).reshape((-1, 1, 2)),
            numpy.linalg.inv(M),
        )[:, 0, :2]
    ...
    if do_debug:
        ...
        image_trans = numpy.full(
            (int(Rxy[0] * 4), int(Rxy[0] * 4), 3), 32, dtype=numpy.uint8
        )
        ...
        cv2.line(
            image_trans,
            (int(pt1.flatten()[0]) + shift[0], int(pt1.flatten()[1]) + shift[1]),
            (int(pt2.flatten()[0]) + shift[0], int(pt2.flatten()[1]) + shift[1]),
            color=(255, 128, 0),
            thickness=4,
        )
        for point in intersections_trans:
            cv2.circle(
                image_trans,
                (shift[0] + int(point[0]), shift[1] + int(point[1])),
                4,
                (255, 255, 255),
                thickness=-1,
            )
...
def distance_point_ellipse(point, ellipse, do_debug=False):
    ...
    if do_debug:
        ...
        image_trans = numpy.full(
            (int(Rxy[1] * 4), int(Rxy[0] * 4), 3), 32, dtype=numpy.uint8
        )
        shift = numpy.array((200, 200))
        cv2.ellipse(
            image_trans,
        ...
        cv2.circle(
            image_trans,
            (new_point.flatten()[0] + shift[0], new_point.flatten()[1] + shift[1]),
            4,
            (255, 255, 255),
            thickness=-1,
        )
...
def trim_line_by_box(line, box):
    ...
    if (not is_1_in) and (is_2_in):
        for segm in segments:
            p1, p2, d = distance_between_lines(
                segm, line, clampA0=True, clampA1=True, clampB0=False, clampB1=True
            )
            if numpy.abs(d) < tol:
    ...
    if (not is_2_in) and (is_1_in):
        for segm in segments:
            p1, p2, d = distance_between_lines(
                segm, line, clampA0=True, clampA1=True, clampB0=True, clampB1=False
            )
            if numpy.abs(d) < tol:
                result[2], result[3] = p2[0], p2[1]
                return result

    d12 = numpy.linalg.norm(
        numpy.array((line[0], line[1])) - numpy.array((line[2], line[3]))
    )
    ...
    for segm in segments:
        ...
        if numpy.abs(d) < tol and (p1 is not None) and (p2 is not None):
            ...
            d1 = numpy.linalg.norm(
                numpy.array((p1[0], p1[1])) - numpy.array((line[0], line[1]))
            )
            d2 = numpy.linalg.norm(
                numpy.array((p1[0], p1[1])) - numpy.array((line[2], line[3]))
            )
...
def get_ratio_4_lines(vert1, horz1, vert2, horz2, do_debug=False):
    ...
    if do_debug:
        ...
        cv2.line(
            image,
            (int(vert1[0]), int(vert1[1])),
            (int(vert1[2]), int(vert1[3])),
            color_red,
            thickness=4,
        )
        cv2.line(
            ...
            (int(vert2[2]), int(vert2[3])),
            color_red,
            thickness=4,
        )
        cv2.line(
            image,
            (int(horz1[0]), int(horz1[1])),
            (int(horz1[2]), int(horz1[3])),
            color_blue,
            thickness=4,
        )
        cv2.line(
            image,
            (int(horz2[0]), int(horz2[1])),
            (int(horz2[2]), int(horz2[3])),
            color_blue,
            thickness=4,
        )

        image = tools_draw_numpy.draw_convex_hull(
            image, rect, color=color_gray, transperency=0.25
        )
...
# ----------------------------------------------------------------------------------------------------------------------
def get_inverce_perspective_mat(
    image, point_vanishing, line_up, line_bottom, target_width, target_height
):
    if line_bottom is None:
        line_bottom = numpy.array(
            (0, image.shape[0], image.shape[1], image.shape[0]), dtype=numpy.float32
        )
    ...
    p1b = line_intersection(
        line_up, (point_vanishing[0], point_vanishing[1], p3a[0], p3a[1])
    )
    if p1a[0] < p1b[0]:
        p1 = p1a
    else:
    ...
    p2b = line_intersection(
        line_up, (point_vanishing[0], point_vanishing[1], p4a[0], p4a[1])
    )
    if p2a[0] > p2b[0]:
        p2 = p2a
    else:
    ...
    M = get_four_point_transform_mat(
        numpy.array((p1, p2, p3, p4), dtype=numpy.float32), target_width, target_height
    )
...
# ---------------------------------------------------------------------------------------------------------------------
def get_inverce_perspective_mat_v2(
    image, target_W, target_H, point_van_xy, tol_up=100, pad_left=0, pad_right=0
):
    H, W = image.shape[:2]
    ...
    dst = numpy.array(
        [(0, 0), (target_W, 0), (0, target_H), (target_W, target_H)],
        dtype=numpy.float32,
    )
...
def regularize_rect(point_2d, inlined=True, do_debug=False):
    ...
    for a in a_range:
        R = numpy.array(
            [
                [math.cos(a * numpy.pi / 180), -math.sin(a * numpy.pi / 180)],
                [math.sin(a * numpy.pi / 180), math.cos(a * numpy.pi / 180)],
            ]
        )
        ...
        pp = numpy.array(
            [
                R.dot(x)
                for x in numpy.array(((d21, d11), (d21, d12), (d22, d11), (d22, d12)))
            ]
        )
        ...
        if do_debug:
            image = tools_draw_numpy.draw_convex_hull(
                empty, 150 * point_2d_centred + 500, color=(0, 0, 255), transperency=0.5
            )
            image = tools_draw_numpy.draw_convex_hull(
                image, 150 * pp + 500, color=(255, 200, 0), transperency=0.5
            )

            image = tools_draw_numpy.draw_lines(
                image, [50 * line1 + 500], (64, 64, 64), w=1
            )
            image = tools_draw_numpy.draw_lines(
                image, [50 * line2 + 500], (64, 64, 64), w=1
            )
            cv2.imwrite("./images/output/xxx_%05d.png" % int(10 * loss), image)

    result = (
        numpy.mean(point_2d, axis=0)
        + numpy.array(results)[numpy.argmin(numpy.array(losses))]
    )
