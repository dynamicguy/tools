...
# ----------------------------------------------------------------------------------------------------------------------
def get_precsion_recall_data_from_markups(
    folder_annotation,
    file_markup_true,
    file_markup_pred,
    iuo_th,
    ...
    for line in lines_true:
        dict_classes[int(line.split(delim)[5])] = 0
    for line in lines_pred:
        dict_classes[int(line.split(delim)[5])] = 0
...
def write_precision_recall(
    filename_out, precision, recall, caption="", color=(128, 128, 128)
):
...
def plot_mAP_iou(
    ...
    (
        ...
        obj_threshold,
        nms_threshold,
    ) = tools_YOLO.load_metadata(filename_meta)
    ...
    for iuo_th in iuo_ths:
        ...
        (
            ...
            confidences,
            class_IDs,
        ) = get_precsion_recall_data_from_markups(
            folder_annotation,
            file_markup_true,
            file_markup_pred,
            iuo_th,
        ...
        for i, class_ID in enumerate(class_IDs):
            if len(precisions[i]) > 1:
                filename_out = (
                    out_dir
                    + out_prefix
                    + "AP_%02d_%s.png" % (class_ID, class_names[class_ID])
                )
    ...
    for b, filename in enumerate(set(file_true)):
        ...
        for coord, hit, conf in zip(coord_pred[idx], hit_pred[idx], conf_pred[idx]):
            if conf < confidence:
                hit = 0
                cv2.rectangle(
                    image, (coord[0], coord[1]), (coord[2], coord[3]), gray, thickness=1
                )
            else:
                cv2.rectangle(
                    image,
