...
def convert_coordinates(tensor, start_index, conversion, border_pixels="half"):
    ...
    if conversion == "minmax2centroids":
        ...
        tensor1[..., ind + 1] = (
            tensor[..., ind] + tensor[..., ind + 2] / 2.0
        )  # Set xmax
        tensor1[..., ind + 2] = (
            tensor[..., ind + 1] - tensor[..., ind + 3] / 2.0
        )  # Set ymin
        tensor1[..., ind + 3] = (
            tensor[..., ind + 1] + tensor[..., ind + 3] / 2.0
        )  # Set ymax
...
def convert_coordinates2(tensor, start_index, conversion):
    ...
    if conversion == "minmax2centroids":
        ...
        )  # The multiplicative inverse of the matrix above
        tensor1[..., ind : ind + 4] = np.dot(tensor1[..., ind : ind + 4], M)
    else:
        raise ValueError(
            "Unexpected conversion value. Supported values are 'minmax2centroids' and 'centroids2minmax'."
        )
...
def intersection_area(
    boxes1, boxes2, coords="centroids", mode="outer_product", border_pixels="half"
):
...
# ----------------------------------------------------------------------------------------------------------------------
def intersection_area_(
    boxes1, boxes2, coords="corners", mode="outer_product", border_pixels="half"
):
...
def iou(boxes1, boxes2, coords="centroids", mode="outer_product", border_pixels="half"):
    ...
    if not mode in {"outer_product", "element-wise"}:
        raise ValueError(
            "`mode` must be one of 'outer_product' and 'element-wise', but got '{}'.".format(
                mode
            )
        )
...
class AnchorBoxes(Layer):
    ...
    def call(self, x, mask=None):
            ...
            if ar == 1:
                ...
                if self.two_boxes_for_ar1:
                    ...
                    box_height = box_width = (
                        np.sqrt(self.this_scale * self.next_scale) * size
                    )
                    wh_list.append((box_width, box_height))
            else:
        ...
        if self.this_steps is None:
            ...
            if isinstance(self.this_steps, (list, tuple)) and (
                len(self.this_steps) == 2
            ):
        ...
        if self.this_offsets is None:
            ...
            if isinstance(self.this_offsets, (list, tuple)) and (
                len(self.this_offsets) == 2
            ):
        ...
        boxes_tensor = K.tile(
            K.constant(boxes_tensor, dtype="float32"), (K.shape(x)[0], 1, 1, 1, 1)
        )
...
class DecodeDetections(Layer):
    ...
    def call(self, y_pred, mask=None):
        ...
        cx = (
            y_pred[..., -12] * y_pred[..., -4] * y_pred[..., -6] + y_pred[..., -8]
        )  # cx = cx_pred * cx_variance * w_anchor + cx_anchor
        cy = (
            y_pred[..., -11] * y_pred[..., -3] * y_pred[..., -5] + y_pred[..., -7]
        )  # cy = cy_pred * cy_variance * h_anchor + cy_anchor
        w = (
            tf.exp(y_pred[..., -10] * y_pred[..., -2]) * y_pred[..., -6]
        )  # w = exp(w_pred * variance_w) * w_anchor
        h = (
            tf.exp(y_pred[..., -9] * y_pred[..., -1]) * y_pred[..., -5]
        ...
        def filter_predictions(batch_item):
            ...
            def filter_single_class(index):
                ...
                def perform_nms():
                    ...
                    maxima = tf.gather(
                        params=single_class, indices=maxima_indices, axis=0
                    )
                    return maxima
                ...
                single_class_nms = tf.cond(
                    ...
                    no_confident_predictions,
                    perform_nms,
                )

                # Make sure `single_class` is exactly `self.nms_max_output_size` elements long.
                padded_single_class = tf.pad(
                    tensor=single_class_nms,
                    paddings=[
                        [
                            0,
                            self.tf_nms_max_output_size - tf.shape(single_class_nms)[0],
                        ],
                        [0, 0],
                    ],
                    mode="CONSTANT",
            ...
            filtered_single_classes = tf.map_fn(
                fn=lambda i: filter_single_class(i),
            ...
            def top_k():
                return tf.gather(
                    params=filtered_predictions,
                    indices=tf.nn.top_k(
                        filtered_predictions[:, 1], k=self.tf_top_k, sorted=True
                    ).indices,
                    axis=0,
                )
            ...
            def pad_and_top_k():
                padded_predictions = tf.pad(
                    tensor=filtered_predictions,
                    paddings=[
                        [0, self.tf_top_k - tf.shape(filtered_predictions)[0]],
                        [0, 0],
                    ],
                    mode="CONSTANT",
                    constant_values=0.0,
                ...
                return tf.gather(
                    params=padded_predictions,
                    indices=tf.nn.top_k(
                        padded_predictions[:, 1], k=self.tf_top_k, sorted=True
                    ).indices,
                    axis=0,
                )

            top_k_boxes = tf.cond(
                tf.greater_equal(tf.shape(filtered_predictions)[0], self.tf_top_k),
                top_k,
                pad_and_top_k,
            )
        ...
        output_tensor = tf.map_fn(
            fn=lambda x: filter_predictions(x),
            elems=y_pred,
            dtype=None,
            parallel_iterations=128,
            back_prop=False,
...
class DecodeDetectionsFast(Layer):
    ...
    def call(self, y_pred, mask=None):
        ...
        class_ids = tf.expand_dims(
            tf.to_float(tf.argmax(y_pred[..., :-12], axis=-1)), axis=-1
        )
        ...
        def filter_predictions(batch_item):
            ...
            predictions = tf.boolean_mask(tensor=batch_item, mask=positive_boxes)
            ...
            predictions_conf_thresh = tf.cond(
                tf.equal(tf.size(predictions), 0),
                no_positive_boxes,
            ...
            def perform_nms():
                ...
                maxima = tf.gather(
                    params=predictions_conf_thresh, indices=maxima_indices, axis=0
                )
                return maxima
            ...
            def top_k():
                return tf.gather(
                    params=predictions_nms,
                    indices=tf.nn.top_k(
                        predictions_nms[:, 1], k=self.tf_top_k, sorted=True
                    ).indices,
                    axis=0,
                )
            ...
            def pad_and_top_k():
                padded_predictions = tf.pad(
                    ...
                    paddings=[
                        [0, self.tf_top_k - tf.shape(predictions_nms)[0]],
                        [0, 0],
                    ],
                    mode="CONSTANT",
                    constant_values=0.0,
...
class SSDLoss:
    ...
    def smooth_L1_loss(self, y_true, y_pred):
        ...
        l1_loss = tf.where(
            tf.less(absolute_loss, 1.0), square_loss, absolute_loss - 0.5
        )
    ...
    def compute_loss(self, y_true, y_pred):
        ...
        positives = tf.to_float(
            tf.reduce_max(y_true[:, :, 1:-12], axis=-1)
        ...
        n_negative_keep = tf.minimum(
            tf.maximum(self.neg_pos_ratio * tf.to_int32(n_positive), self.n_neg_min),
            n_neg_losses,
        )
        ...
        def f2():
            ...
            neg_class_loss_all_1D = tf.reshape(
                neg_class_loss_all, [-1]
            )  # Tensor of shape (batch_size * n_boxes,)
            # ...and then we get the indices for the `n_negative_keep` boxes with the highest loss out of those...
            values, indices = tf.nn.top_k(
                neg_class_loss_all_1D, k=n_negative_keep, sorted=False
            )  # We don't need them sorted.
        ...
        loc_loss = tf.reduce_sum(
            localization_loss * positives, axis=-1
        )  # Tensor of shape (batch_size,)
...
def match_multi(weight_matrix, threshold):
    ...
    all_anchor_indices = list(
        range(num_anchor_boxes)
    )  # Only relevant for fancy-indexing below.

    # Find the best ground truth match for every anchor box.
    ground_truth_indices = np.argmax(
        weight_matrix, axis=0
    )  # Array of shape (weight_matrix.shape[1],)
    overlaps = weight_matrix[
        ground_truth_indices, all_anchor_indices
    ]  # Array of shape (weight_matrix.shape[1],)
...
class SSDInputEncoder:
    def __init__(
        ...
        if scales:
            if (
                len(scales) != predictor_sizes.shape[0] + 1
        ...
        if not (aspect_ratios_per_layer is None):
            if (
                len(aspect_ratios_per_layer) != predictor_sizes.shape[0]
            ):  # Must be two nested `if` statements since `list` and `bool` cannot be combined by `&`
                raise ValueError(
    ...
    def __call__(self, ground_truth_labels, diagnostics=False):
        ...
        y_encoded[
            :, :, self.background_id
        ] = 1  # All boxes are background boxes by default.
        ...
        for i in range(batch_size):  # For each batch item...
            ...
            classes_one_hot = class_vectors[
                labels[:, class_id].astype(np.int)
            ...
            neutral_boxes = np.nonzero(
                max_background_similarities >= self.neg_iou_limit
            )[0]
        ...
        if diagnostics:
            ...
            y_matched_anchors[
                :, :, -12:-8
            ] = 0  # Keeping the anchor box coordinates means setting the offsets to zero.
...
def ssd_300(
    ...
    pool1 = MaxPooling2D(
        pool_size=(2, 2), strides=(2, 2), padding="same", name="pool1"
    )(conv1_2)
    ...
    pool2 = MaxPooling2D(
        pool_size=(2, 2), strides=(2, 2), padding="same", name="pool2"
    )(conv2_2)
    ...
    pool3 = MaxPooling2D(
        pool_size=(2, 2), strides=(2, 2), padding="same", name="pool3"
    )(conv3_3)
    ...
    pool4 = MaxPooling2D(
        pool_size=(2, 2), strides=(2, 2), padding="same", name="pool4"
    )(conv4_3)
    ...
    pool5 = MaxPooling2D(
        pool_size=(3, 3), strides=(1, 1), padding="same", name="pool5"
    )(conv5_3)
