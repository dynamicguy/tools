...
def get_K_RT(points_3d, points_2d, image):
    ...
    matrix_init = tools_pr_geom.compose_projection_mat_3x3(
...
def get_proj_dist_mat_for_image(filename, chess_rows, chess_cols):
    ...
    world_points = numpy.hstack(
        (
            x.reshape(chess_rows * chess_cols, 1),
            y.reshape(chess_rows * chess_cols, 1),
            numpy.zeros((chess_rows * chess_cols, 1)),
        )
    ...
    if ret:
        ...
        ret, cameraMatrix, distCoeffs, rvecs, tvecs = cv2.calibrateCamera(
            _3d_points, _2d_points, (im.shape[1], im.shape[0]), None, None
        )
        cv_im_undistorted = cv2.undistort(im, cameraMatrix, distCoeffs)
        projectPoints = numpy.array(
            cv2.projectPoints(
                world_points,
                numpy.array(rvecs),
                numpy.array(tvecs),
        ...
        projectPoints = projectPoints.reshape(
            projectPoints.shape[0], projectPoints.shape[2]
        )
...
# ----------------------------------------------------------------------------------------------------------------------
def get_proj_dist_mat_for_images(
    folder_in, chess_rows, chess_cols, cell_size=1, folder_out=None
):
    ...
    for local_filename in tools_IO.get_filenames(folder_in, "*.png,*.jpg"):
        ...
        if ret:
            ...
            for i in range(0, corners.shape[0]):
                gray = tools_draw_numpy.draw_circle(
                    gray, corners[i, 1], corners[i, 0], 3, [0, 0, 255], alpha_transp=0
                )
    ...
    print(camera_matrix)
    ret, camera_matrix, dist, rvecs, tvecs = cv2.calibrateCamera(
        cell_size * numpy.array(_3d_points),
        _2d_points,
        (image.shape[1], image.shape[0]),
        matrix_init,
        dist_init,
...
def rectify_pair(mtx, dist, image1, image2, chess_rows, chess_cols):
    ...
    if ret1 and ret2:
        ...
        retval, _, _, _, _, R, T, E, F = cv2.stereoCalibrate(
            ...
            mtx,
            dist,
            (gray_image1.shape[1], gray_image1.shape[0]),
            flags=flg,
        )
...
# ---------------------------------------------------------------------------------------------------------------------
def get_stitched_images_using_homography(
    img1, img2, M, background_color=(255, 255, 255), borderMode=cv2.BORDER_CONSTANT
):
    ...
    img1_dims_temp = numpy.float32([[0, 0], [0, w1], [h1, w1], [h1, 0]]).reshape(
        -1, 1, 2
    )
    img1_dims = cv2.perspectiveTransform(
        img1_dims_temp, M
    )  # Get relative perspective of second image
    result_dims = numpy.concatenate(
        (img1_dims, img2_dims), axis=0
    ...
    result_img_1 = cv2.warpPerspective(
        img1,
    ...
    result_img_2[
        transform_dist[1] : w2 + transform_dist[1],
        transform_dist[0] : h2 + transform_dist[0],
    ] = img2
    if borderMode == cv2.BORDER_REPLICATE:
        result_img_2 = tools_image.fill_border(
            result_img_2,
            transform_dist[1],
            transform_dist[0],
            w2 + transform_dist[1],
...
def homography_coordinates(img1, img2, M, coord1, coord2):
    ...
    x = (H[0, 0] * coord1[:, 0] + H[0, 1] * coord1[:, 1] + H[0, 2] * 1) / (
        H[2, 0] * coord1[:, 0] + H[2, 1] * coord1[:, 1] + H[2, 2] * 1
    )
    y = (H[1, 0] * coord1[:, 0] + H[1, 1] * coord1[:, 1] + H[1, 2] * 1) / (
        H[2, 0] * coord1[:, 0] + H[2, 1] * coord1[:, 1] + H[2, 2] * 1
    )
...
# ---------------------------------------------------------------------------------------------------------------------
def get_stitched_images_using_translation(
    img1,
    img2,
    translation,
    ...
    if keep_shape == False:
        if borderMode == cv2.BORDER_REPLICATE:
            result_img2 = tools_image.fill_border(
                result_img2,
                transform_dist[1],
                transform_dist[0],
                w2 + transform_dist[1],
        ...
        result_img2 = result_img2[
            transform_dist[1] : transform_dist[1] + img2.shape[0],
            transform_dist[0] : transform_dist[0] + img2.shape[1],
        ]
...
# ---------------------------------------------------------------------------------------------------------------------
def get_transform_by_keypoints_desc(
    points_source, des_source, points_destin, des_destin, matchtype="knn"
):

    M = None
    if (
        points_source is None
        or des_source is None
        or points_destin is None
        or des_destin is None
    ):
        return M

    src, dst, distance = tools_alg_match.get_matches_from_keypoints_desc(
        points_source, des_source, points_destin, des_destin, matchtype=matchtype
    )
...
# --------------------------------------------------------------------------------------------------------------------------
def get_homography_by_keypoints_desc(
    points_source, des_source, points_destin, des_destin, matchtype="knn"
):
...
def is_homography_good(src, dst, M):
    ...
    dims = numpy.float32([[0, 0], [0, src_w], [src_h, src_w], [src_h, 0]]).reshape(
        -1, 1, 2
    )
    ...
    for i in range(dims2.shape[0]):
        ...
        if dims2[i][0][1] < -10:
            return False
        if dims2[i][0][0] > dst_w + 10:
            return False
        if dims2[i][0][1] > dst_h + 10:
            return False
...
def is_transform_good(src, dst, M):
    ...
    for i in range(dims2.shape[0]):
        ...
        if dims2[i][0][1] < -10:
            return False
        if dims2[i][0][0] > dst_h + 10:
            return False
        if dims2[i][0][1] > dst_w + 10:
            return False
...
def derive_transform(
    img1, img2, K=numpy.array([[1000, 0, 0], [0, 1000, 0], [0, 0, 1]])
):
...
def compose_homography(
    R, T, normal, K=numpy.array([[1000, 0, 0], [0, 1000, 0], [0, 0, 1]])
):
...
# ----------------------------------------------------------------------------------------------------------------------
def get_inverse_homography_from_RT(
    R, T, normal, K=numpy.array([[1000, 0, 0], [0, 1000, 0], [0, 0, 1]])
):
...
# ---------------------------------------------------------------------------------------------------------------------
def align_two_images_translation(
    img1,
    img2,
    detector="SIFT",
    matchtype="knn",
    ...
    if borderMode == cv2.BORDER_REPLICATE:
        result_image1, result_image2 = get_stitched_images_using_translation(
            img1, img2, translation, borderMode=cv2.BORDER_REPLICATE, keep_shape=True
        )
    else:
    ...
    q = (
        (
            cv2.matchTemplate(
                result_image1, result_image2, method=cv2.TM_CCOEFF_NORMED
            )[0, 0]
        )
        + 1
    ) * 128
...
def align_two_images_homography(img1, img2, detector="SIFT", matchtype="knn"):
    ...
    homography = None
    if match1.size != 0:
        homography = get_homography_by_keypoints_desc(
            points1, des1, points2, des2, matchtype
        )
    ...
    for each in match1:
        cv2.circle(
            img1_gray_rgb, (int(each[0]), int(each[1])), 3, [0, 0, 255], thickness=-1
        )
    for each in match2:
        cv2.circle(
            img2_gray_rgb, (int(each[0]), int(each[1])), 3, [255, 255, 0], thickness=-1
        )
    ...
    q = cv2.matchTemplate(result_image1, result_image2, method=cv2.TM_CCOEFF_NORMED)[
        0, 0
    ]
    q = int((1 + q) * 128)
...
def pose_estimation_chessboard(img, chess_rows, chess_cols, cameraMatrix, dist):
    ...
    if ret == True:
        ...
        _, rvecs, tvecs, inliers = cv2.solvePnPRansac(
            corners_3d, corners_2d, cameraMatrix, dist
        )
