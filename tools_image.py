...
def canvas_extrapolate_gray(gray, new_height, new_width):
    ...
    newimage[
        shift_x : shift_x + gray.shape[0], shift_y : shift_y + gray.shape[1]
    ] = gray[:, :]
    ...
    for row in range(0, newimage.shape[0]):
        ...
        newimage[row, shift_y + gray.shape[1] :] = newimage[
            row, shift_y + gray.shape[1] - 1
        ]
...
# ---------------------------------------------------------------------------------------------------------------------
def smart_resize(
    img, target_image_height, target_image_width, bg_color=(128, 128, 128)
):
    ...
    scale = min(
        target_image_width / original_image_width,
        target_image_height / original_image_height,
    )
    ...
    new_image.paste(
        pillow_image, ((target_image_width - nw) // 2, (target_image_height - nh) // 2)
    )
...
def center_crop(img, size):
    ...
    if h > w:
        ...
        return crop_image(
            ...
            left=0,
            bottom=new_height - 1 - (new_height - new_width) // 2,
            right=new_width,
        )
    else:
        new_height = size
        ...
        return crop_image(
            ...
            bottom=new_height,
            right=new_width - 1 - (new_width - new_height) // 2,
        )


# ---------------------------------------------------------------------------------------------------------------------
def smart_resize_point(
    original_x, original_y, original_w, original_h, target_w, target_h
):
...
def canvas_extrapolate(img, new_height, new_width):
    ...
    for row in range(0, newimage.shape[0]):
        newimage[row, :shift_y] = newimage[row, shift_y]
        newimage[row, shift_y + img.shape[1] :] = newimage[
            row, shift_y + img.shape[1] - 1
        ]
...
def fade_header(img, color, top):
    ...
    for row in range(top):
        ...
        for col in range(img.shape[1]):
            newimage[row, col, 0] = (
                newimage[row, col, 0] * alpha + (1 - alpha) * color[0]
            )
            newimage[row, col, 1] = (
                newimage[row, col, 1] * alpha + (1 - alpha) * color[1]
            )
            newimage[row, col, 2] = (
                newimage[row, col, 2] * alpha + (1 - alpha) * color[2]
            )
...
def fade_left_right(img, left, right):
    ...
    for col in range(right, img.shape[1]):
        ...
        for row in range(img.shape[0]):
            newimage[row, col, 0] = (
                newimage[row, col, 0] * alpha
                + (1 - alpha) * avg[row, img.shape[1] - 1, 0]
            )
            newimage[row, col, 1] = (
                newimage[row, col, 1] * alpha
                + (1 - alpha) * avg[row, img.shape[1] - 1, 1]
            )
            newimage[row, col, 2] = (
                newimage[row, col, 2] * alpha
                + (1 - alpha) * avg[row, img.shape[1] - 1, 2]
            )
    ...
    for col in range(0, left):
        ...
        for row in range(img.shape[0]):
            newimage[row, col, 0] = (
                newimage[row, col, 0] * alpha + (1 - alpha) * avg[row, 0, 0]
            )
            newimage[row, col, 1] = (
                newimage[row, col, 1] * alpha + (1 - alpha) * avg[row, 0, 1]
            )
            newimage[row, col, 2] = (
                newimage[row, col, 2] * alpha + (1 - alpha) * avg[row, 0, 2]
            )
...
def crop_image(img, top, left, bottom, right, extrapolate_border=False):
    ...
    result[
        start_row_target:finish_row_target, start_col_target:finish_col_target
    ] = img[start_row_source:finish_row_source, start_col_source:finish_col_source]
...
def put_layer_on_image(image_background, image_layer, background_color=(0, 0, 0)):
    ...
    img1 = cv2.bitwise_and(
        image_background, image_background, mask=mask_layer_inv
    ).astype(numpy.uint8)
    img2 = cv2.bitwise_and(image_layer, image_layer, mask=mask_layer).astype(
        numpy.uint8
    )
...
def put_image(image_large, image_small, start_row, start_col):
    ...
    if (
        start_col < end_col_large
        and start_row < end_row_large
        and end_row_large >= 0
        and end_col_large >= 0
    ):
        result[start_row:end_row_large, start_col:end_col_large] = image_small[
            start_row_small:end_row_small, start_col_small:end_col_small
        ]
    return result
...
def hsv2bgr(hsv):
    return cv2.cvtColor(
        numpy.array([hsv[0], hsv[1], hsv[2]], dtype=numpy.uint8).reshape(1, 1, 3),
        cv2.COLOR_HSV2BGR,
    )
...
def bgr2hsv(brg):
    return cv2.cvtColor(
        numpy.array([brg[0], brg[1], brg[2]], dtype=numpy.uint8).reshape(1, 1, 3),
        cv2.COLOR_BGR2HSV,
    )
...
def gre2jet(rgb):
    return cv2.applyColorMap(
        numpy.array(rgb, dtype=numpy.uint8).reshape((1, 1, 3)), cv2.COLORMAP_JET
    ).reshape(3)
...
def gre2viridis(rgb):
    colormap = numpy.flip(
        (numpy.array(cm.cmaps_listed["viridis"].colors) * 256).astype(int), axis=1
    )
...
def hitmap2d_to_colormap(hitmap_2d, cmap=plt.cm.Set3, interpolate_colors=False):
    ...
    if not interpolate_colors:
        ...
        for i in range(N):
            ...
            colors.append(
                numpy.array(cmap(i1)) * (1 - alpha)
                + numpy.array(cmap(i1 + 1)) * (alpha)
            )
        colors = 255 * numpy.array(colors)[:, [2, 1, 0]]

    hitmap_RGB = numpy.zeros((hitmap_2d.shape[0], hitmap_2d.shape[1], 3)).astype(
        numpy.uint8
    )
...
# ----------------------------------------------------------------------------------------------------------------------
def blend_multi_band_large_small0(
    large,
    small,
    ...
    if do_debug:
        cv2.imwrite("./images/output/mask0.png", 255 * mask)

    if n_clips > 0:
        mask = ndimage.uniform_filter(
            mask, size=(filter_size, filter_size), mode="reflect"
        )
    ...
    if adjust_colors is not None:
        ...
        if len(idx[0]) > 0:
            for c in range(3):
                scale = numpy.average(small[:, :, c][idx]) / numpy.average(
                    large[:, :, c][idx]
                )
    ...
    if do_debug:
        cv2.imwrite("./images/output/small_corrected.png", small)
...
# ----------------------------------------------------------------------------------------------------------------------
def blend_multi_band_large_small(
    large,
    small,
    background_color=(255, 255, 255),
    adjust_colors=False,
    ...
    if adjust_colors:
        ...
        cnt_small = sliding_2d(
            1 - mask_bin, -filter_size, filter_size, -filter_size, filter_size, "cnt"
        )
        for c in range(3):

            avg_large = sliding_2d(
                large[:, :, c],
                -filter_size,
                filter_size,
                -filter_size,
            ...
            if do_debug:
                cv2.imwrite("./images/output/avg_large.png", avg_large)
            if do_debug:
                cv2.imwrite("./images/output/avg_small.png", avg_small)
            if do_debug:
                cv2.imwrite("./images/output/cnt_small.png", cnt_small)
...
def convolve_with_mask_fft(image255, mask_pn, min_value=None, max_value=None):
    ...
    res = numpy.real(
        numpy.fft.ifft2(
            numpy.fft.fft2(image255) * numpy.fft.fft2(mask_pn, s=image255.shape)
        )
    )
    res = res[KW:, KW:]
...
def do_patch(img, patch_size=128, boarder=12, color=True):
    ...
    for y in range(0, img_h, OUT_PATCH_SIZE):
        for x in range(0, img_w, OUT_PATCH_SIZE):
            if not color:
                input_patch_im.append(
                    numpy.expand_dims(
                        numpy.asarray(
                            ext_img[y : y + patch_size, x : x + patch_size],
                            dtype=img.dtype,
                        ),
                        axis=2,
                    )
...
def do_stitch(img_h, img_w, patches, boarder=12, color=True):
    ...
    if color:
        output_img = numpy.zeros(
            (img_h + out_patch_size, img_w + out_patch_size, 3), dtype="uint8"
        )
    else:
        output_img = numpy.zeros(
            (img_h + out_patch_size, img_w + out_patch_size, 1), dtype="uint8"
        )
    idx = 0
    for y in range(0, img_h, out_patch_size):
        for x in range(0, img_w, out_patch_size):
            patch = patches[idx, :, :]
            patch = patch[
                boarder : boarder + out_patch_size, boarder : boarder + out_patch_size
            ]
...
def skew_hor(A, value, do_inverce=False):
    ...
    if not do_inverce:
        for r in range(B.shape[0]):
            ...
            if len(shape) == 3:
                B[r, v : v + A.shape[1], [0, 1, 2]] = A[r, :, [0, 1, 2]]
            else:
                B[r, v : v + A.shape[1]] = A[r, :]
    else:
        for r in range(B.shape[0]):
            v = int(value * r / (B.shape[0] - 1))
            if v < 0:
                v += -value
            if len(shape) == 3:
                B[r, :, [0, 1, 2]] = A[r, v : v + B.shape[1], [0, 1, 2]]
            else:
                B[r, :] = A[r, v : v + B.shape[1]]
...
def put_color_by_mask(image, mask2d, color):
    ...
    for r, c in zip(idx[0], idx[1]):
        image[r, c, [0, 1, 2]] = (
            image[r, c, [0, 1, 2]] * (1 - mask2d[r, c] / 255)
            + numpy.array(color)[[0, 1, 2]] * mask2d[r, c] / 255
        )
