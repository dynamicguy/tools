...
class Calibrator(object):
    ...
    def save_points(self, filename_out, IDs, points_2d_all, points_gps_all, W, H):
        ...
        tools_IO.append_CSV(
            [-1, 0, 0, 0, 0, 0], filename_out, csv_header=csv_header, delim="\t"
        )

        for pid, p1, p2 in zip(IDs, points_2d_all, points_gps_all):
            if numpy.any(numpy.isnan(p1)):
                continue
            if numpy.any(numpy.isnan(p2)):
                continue
            if p1[0] < 0 or p1[0] > W:
                continue
            if p1[1] < 0 or p1[1] > H:
    ...
    def shift_scale(self, points_gnss_3d, origin_xy, orignin_z=0, scale_factor_xy=None):
        ...
        if scale_factor_xy is None:
            scale_factor_xy = self.scale_factor_gps
    ...
    def get_adjustment_params(self, marker_xy, target_W, target_H):
        ...
        scale = max(
            (marker_xy[:, 0].max() - marker_xy[:, 0].min()) / target_W,
            (marker_xy[:, 1].max() - marker_xy[:, 1].min()) / target_W,
        )
    ...
    # ----------------------------------------------------------------------------------------------------------------------
    def adjust_2d(
        self, xy, target_W, target_H, scale, shift1, shift2, do_flip_v, shift_h=None
    ):
    ...
    def AR_points(
        ...
        if list_of_R is not None:
            ...
            colors_ln = tools_draw_numpy.get_colors(
                len(list_of_R), colormap=self.colormap_circles
            )[::-1]
        ...
        if virt_obj is not None:
            for p in points_xyz:
                ...
                M = tools_pr_geom.compose_RT_mat(
                    rvecs, tvecs, do_rodriges=True, do_flip=False, GL_style=False
                )
                ...
                image_AR = tools_draw_numpy.draw_cuboid(
                    image_AR,
                    tools_pr_geom.project_points_p3x4(cuboid_3d, numpy.matmul(P, M)),
                )
        ...
        image_AR = tools_draw_numpy.draw_ellipses(
            image_AR,
            [((p[0], p[1]), (25, 25), 0) for p in points_2d],
            color=(0, 0, 190),
            w=4,
            labels=labels,
        )
        image_AR = tools_draw_numpy.draw_points(
            image_AR, points_2d_check, color=(0, 128, 255), w=8
        )
    ...
    def BEV_points(
        ...
        scale, shift1, shift2, do_flip_v, shift_h = self.get_adjustment_params(
            marker_xy[:, :2], target_W, target_H
        )
        if dots_pr_meter is not None:
            scale = 1 / dots_pr_meter
        marker_xy = self.adjust_2d(
            marker_xy[:, :2],
            target_W,
            target_H,
            scale,
        ...
        marker_xy_ext = tools_draw_numpy.extend_view(
            marker_xy, target_H, target_W, factor
        )
        ...
        if cuboids_3d is not None:
            for i, each in enumerate(cuboids_3d.XYZs):
                ...
                cuboids_xy_ext.append(
                    tools_draw_numpy.extend_view(cuboid_xy, target_H, target_W, factor)
                )
                cuboid_IDs.append(i)
        ...
        if list_of_R is not None and len(list_of_R) > 0:
            ...
            RR = -numpy.sort(-numpy.array(list_of_R).flatten())
            center_ext = tools_draw_numpy.extend_view(
                marker_xy[0], target_H, target_W, factor
            )[0]
            colors_GB = numpy.arange(180, 255, (255 - 180) / len(list_of_R))[::-1]

            for rad, color_bg, color_ln in zip(RR, colors_GB, colors_ln):
                image_R = tools_draw_numpy.draw_circle(
                    image_R,
                    center_ext[1],
                    center_ext[0],
                    rad / scale / factor,
                ...
                ellipse = (
                    (center_ext[0], center_ext[1]),
                    ((2 * rad / scale / factor), 2 * rad / scale / factor),
                    0,
                )
                image_R = tools_draw_numpy.draw_ellipses(
                    image_R, [ellipse], color=color_ln.tolist(), w=1
                )
        else:
            colors_ln = tools_draw_numpy.get_colors(1, colormap=self.colormap_circles)[
                ::-1
            ]

        H = self.derive_homography(marker_2d[1:], marker_xy[1:])
        image_warped = cv2.warpPerspective(
            gray, H, (target_W, target_H), borderValue=(255, 255, 255)
        )
        image_warped = tools_draw_numpy.extend_view_from_image(
            image_warped, factor=factor, color_bg=(255, 255, 255)
        )
        image_BEV = tools_image.put_layer_on_image(
            image_R, image_warped, (255, 255, 255)
        )

        points_2d_ext, points_IDs = [], []
        if points_2d is not None:
            for i, each in enumerate(points_2d.XYs):
                point_xy = cv2.perspectiveTransform(
                    each.reshape((-1, 1, 2)).astype(numpy.float32), H
                ).reshape((-1, 2))
                points_2d_ext.append(
                    tools_draw_numpy.extend_view(point_xy, target_H, target_W, factor)
                )
                points_IDs.append(i)
        ...
        if draw_points:
            image_BEV = tools_draw_numpy.draw_points(
                image_BEV, marker_xy_ext[:1], color=colors_ln[-1].tolist(), w=8
            )

        if len(cuboid_IDs) > 0:
            col_obj = tools_draw_numpy.get_colors(
                len(numpy.unique(cuboid_IDs)), colormap=self.colormap_objects
            )
            for i, id in enumerate(numpy.unique(cuboid_IDs)):
                ...
                image_BEV = tools_draw_numpy.draw_contours(
                    image_BEV,
                    p,
                    color_fill=col_obj[i].tolist(),
                    color_outline=col_obj[i].tolist(),
                    transp_fill=0.3,
                    transp_outline=1.0,
                )
                image_BEV = tools_draw_numpy.draw_lines(
                    ...
                    color=col_obj[i].tolist(),
                    w=5,
                )

        if len(points_IDs) > 0:
            col_obj = tools_draw_numpy.get_colors(
                len(numpy.unique(points_IDs)), colormap=self.colormap_objects
            )
            for i, id in enumerate(numpy.unique(points_IDs)):
                p = points_2d_ext[points_IDs == id][0]
                if len(p) == 1:
                    image_BEV = tools_draw_numpy.draw_points(
                        image_BEV, p, col_obj[i].tolist(), w=12
                    )
                else:
                    image_BEV = tools_draw_numpy.draw_contours(
                        image_BEV,
    ...
    def construct_cuboid(self, dim, rvec, tvec):
        ...
        RT = tools_pr_geom.compose_RT_mat(
            rvec, tvec, do_rodriges=True, do_flip=False, GL_style=False
        )
    ...
    def evaluate_fov(
        ...
        for a_fov in a_fovs:
            ...
            err.append(
                numpy.sqrt(((points_2d_check - points_2d) ** 2).sum() / len(points_2d))
            )
        ...
        if do_debug:
            ...
            for i in range(len(a_fovs)):
                ...
                points_2d_check, jac = tools_pr_geom.project_points(
                    points_xyz, rvecs[i], tvecs[i], camera_matrix, numpy.zeros(5)
                )
                image_AR = tools_draw_numpy.draw_ellipses(
                    gray,
                    [((p[0], p[1]), (25, 25), 0) for p in points_2d],
                ...
                image_AR = tools_draw_numpy.draw_points(
                    image_AR, points_2d_check.reshape((-1, 2)), color=color_markup, w=8
                )
                ...
                cv2.imwrite(
                    self.folder_out + base_name + "_%05d" % (a_fovs[i] * 1000) + ".png",
                    image_AR,
                )

        return (
            a_fovs[idx_best],
            numpy.array(rvecs)[idx_best],
            numpy.array(tvecs)[idx_best],
        )
    ...
    def get_pretty_model_rotation(self, mat_model):
        ...
        mat_model_check3 = tools_pr_geom.compose_RT_mat(
            rvec_check, (0, 0, 0), do_rodriges=False, do_flip=False
        )

        return a_pitch_deg, a_yaw_deg, a_roll_deg

    # ----------------------------------------------------------------------------------------------------------------------
    def evaluate_matrices_GL(
        self, image, rvec, tvec, a_fov, points_3d=None, virt_obj=None, do_debug=False
    ):

        if numpy.any(numpy.isnan(rvec)) or numpy.any(numpy.isnan(tvec)):
            return (
                numpy.full((4, 4), numpy.nan),
                numpy.full((4, 4), numpy.nan),
                numpy.full((4, 4), numpy.nan),
            )
        H, W = image.shape[:2]

        mR = tools_pr_geom.compose_RT_mat(
            rvec, (0, 0, 0), do_rodriges=True, do_flip=True, GL_style=True
        )
        mRT = tools_pr_geom.compose_RT_mat(
            rvec, tvec, do_rodriges=True, do_flip=True, GL_style=True
        )
        ...
        if do_debug:
            ...
            R = tools_GL3D.render_GL3D(
                filename_obj=filename_obj,
                W=W,
                H=H,
            ...
            cv2.imwrite(
                ...
                R.get_image_perspective(
                    rvec, tvec, a_fov, a_fov, mat_view_to_1=False, do_debug=True
                ),
            )
            cv2.imwrite(
                self.folder_out + "AR0_GL_v1.png",
                R.get_image_perspective(
                    rvec, tvec, a_fov, a_fov, mat_view_to_1=True, do_debug=True
                ),
            )
            cv2.imwrite(
                self.folder_out + "AR0_CV_cube.png",
                tools_render_CV.draw_cube_numpy_MVP_GL(
                    gray,
                    mat_projection,
                    numpy.eye(4),
                    mRT,
            ...
            cv2.imwrite(
                ...
                tools_render_CV.draw_points_numpy_MVP_GL(
                    points_3d,
                    gray,
                    mat_projection,
                    numpy.eye(4),
            ...
            cv2.imwrite(
                ...
                tools_render_CV.draw_cube_numpy_MVP_GL(
                    gray, mat_projection, mR, numpy.eye(4), T, points_3d=cuboid_3d
                ),
            )
            cv2.imwrite(
                self.folder_out + "AR1_GL.png",
                R.get_image(
                    mat_view=numpy.eye(4), mat_model=mR, mat_trans=T, do_debug=True
                ),
            )
    ...
    def evaluate_K_chessboard(self, folder_in):
        ...
        (
            ...
            rvecs,
            tvecs,
        ) = tools_calibrate.get_proj_dist_mat_for_images(
            folder_in,
            chess_rows,
            chess_cols,
            cell_size=0.03,
    ...
    def unit_test_gnss(self):
        ...
        points_xy = numpy.array(
            [(200, 591), (317, 84), (657, 1075), (381, 952)], numpy.float32
        )
        points_gnss = numpy.array(
            [
                (52.228391, 21.01075),
                (52.227676, 21.000511),
                (52.227384, 21.001689),
    ...
    # ----------------------------------------------------------------------------------------------------------------------
    def derive_cuboid_3d(
        self, mat_camera_3x3, r_vec, t_vec, points_2D, LWH, base_name="", do_debug=False
    ):

        target_centr_top = numpy.mean(points_2D[[0, 2, 4, 6]], axis=0)
        target_centr_bottom = numpy.mean(points_2D[[1, 3, 5, 7]], axis=0)
        target_centr_bottom_3D = tools_pr_geom.reverce_project_points_Z0(
            [target_centr_bottom], r_vec, t_vec, mat_camera_3x3, numpy.zeros(5)
        )[0]

        diffs, Hs = [], numpy.arange(0, 10, 0.1)
        for h in Hs:
            centr_top_2D, _ = tools_pr_geom.project_points(
                target_centr_bottom_3D - numpy.array((0, 0, h)),
                r_vec,
                t_vec,
                mat_camera_3x3,
                numpy.zeros(5),
        ...
        points_3D = tools_pr_geom.reverce_project_points_Z0(
            points_2D[[1, 3, 5, 7]], r_vec, t_vec, mat_camera_3x3, numpy.zeros(5)
        )
        ...
        if do_debug:
            sources_2D, _ = tools_pr_geom.project_points(
                points_3D, r_vec, t_vec, mat_camera_3x3, numpy.zeros(5)
            )
            image_debug = tools_draw_numpy.draw_cuboid(
                numpy.full((1080, 1920, 3), 32, dtype=numpy.uint8),
                points_2D,
                color=(0, 0, 200),
                put_text=True,
            )
            image_debug = tools_draw_numpy.draw_cuboid(
                image_debug, sources_2D, color=(255, 140, 0), put_text=True
            )
    ...
    def centralize_points(
        ...
        M_GL = tools_pr_geom.compose_RT_mat(
            rvec, tvec, do_rodriges=True, do_flip=True, GL_style=True
        )
        P = tools_pr_geom.compose_projection_mat_4x4(
            mat_camera[0, 0],
        ...
        for a_yaw in a_yaws:
            R_CV = tools_pr_geom.compose_RT_mat(
                (0, 0, a_yaw * numpy.pi / 180),
                (0, 0, 0),
                do_rodriges=True,
                do_flip=False,
                GL_style=False,
            ...
            points_2d = tools_pr_geom.project_points_p3x4(
                cuboid_r_CV, numpy.matmul(P, M), check_reverce=True
            )
        ...
        R_GL = tools_pr_geom.compose_RT_mat(
            (0, 0, -a_yaw * numpy.pi / 180),
            (0, 0, 0),
            do_rodriges=True,
            do_flip=True,
        ...
        self.save_points(
            filename_points_out, IDs[1:], points_2d_all[1:], points_xyz_t_CV[1:], W, H
        )
