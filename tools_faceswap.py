...
class Face_Swaper(object):
    def __init__(
        ...
        image_actor,
        device="cpu",
        adjust_every_frame=False,
        do_narrow_face=False,
    ):
    ...
    def narrow_face(self, image, landmarks):
        ...
        mask = tools_draw_numpy.draw_convex_hull_cv(
            mask, numpy.array(landmarks[idx]), color=(1, 1, 1)
        )
    ...
    # ---------------------------------------------------------------------------------------------------------------------
    def process_folder_extract_landmarks(
        self,
        folder_in,
        folder_out,
        write_images=True,
        ...
        for local_filename in local_filenames:
            ...
            if write_images:
                ...
                result = self.D.draw_landmarks_v2(
                    image, L_actor, color=(0, 192, 255), del_triangles=del_triangles_A
                )
            ...
            if write_annotation:
                ...
                data = numpy.vstack(
                    (
                        numpy.array([local_filename, local_filename]),
                        (L_actor).astype(numpy.chararray),
                    )
                ).T
                numpy.savetxt(
                    myfile,
                    data.astype(numpy.str),
                    fmt="%s",
                    encoding="str",
                    delimiter=delim,
                )
    ...
    def process_folder_faceswap(self, filename_clbrt, folder_in, folder_out):
        ...
        for local_filename in local_filenames:
            ...
            result = self.do_faceswap(
                R_c, R_a, image_clbrt, image_actor, L_clbrt, L_actor, del_triangles_C
            )
            cv2.imwrite(folder_out + local_filename, result)
    ...
    # ---------------------------------------------------------------------------------------------------------------------
    def process_folder_draw_landmarks(
        self, folder_in, list_of_filaname_landmarks, folder_out, delim="\t"
    ):
        ...
        for local_filename in filenames_dict:
            if not os.path.isfile(folder_in + local_filename):
                continue
            image = cv2.imread(folder_in + local_filename)
    ...
    # ---------------------------------------------------------------------------------------------------------------------
    def process_folder_faceswap_by_landmarks(
        self, folder_in, filaname_landmarks, folder_out, delim="\t"
    ):
    ...
    def adjust_colors_clbrt(self, avg_size=50):
        ...
        if (
            self.L_actor.min() == self.L_actor.max() == 0
            or self.L_clbrt.min() == self.L_clbrt.max() == 0
        ):
            return
        ...
        for c in range(3):
            ...
            avg_actor = tools_filter.sliding_2d(
                self.image_actor[:, :, c],
                -avg_size,
                avg_size,
                -avg_size,
            ...
            sum_face = tools_filter.sliding_2d(
                face[:, :, c], -avg_size, avg_size, -avg_size, avg_size, "cnt"
            )
        ...
        reverce_scale = R.morph_2D_mesh(
            self.image_clbrt.shape[0],
            self.image_clbrt.shape[1],
            self.L_clbrt[self.D.idx_removed_eyes],
            LA_aligned[self.D.idx_removed_eyes],
            self.del_triangles_C,
        )
    ...
    def do_faceswap_cpu(self, folder_out="./images/output/", do_debug=False):
        ...
        if do_debug:
            cv2.imwrite(folder_out + "s03-face_masked.jpg", face)
        ...
        if do_debug:
            cv2.imwrite(folder_out + "s03-temp_mouth.jpg", temp_mouth)
        ...
        if do_debug:
            cv2.imwrite(folder_out + "s03-result-mouth.jpg", result)
