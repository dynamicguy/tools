...
def generate_input(image_base, coordinates, image_small, border_color=(255, 0, 0)):
    ...
    for i in range(0, coordinates.shape[0]):
        ...
        image_out[dst_left:dst_right, dst_top:dst_bottom, :] = image_small[
...
def build_hitmap(folder_in, filename_in_field, filename_in_pattern, folder_out):
    ...
    cv2.imwrite(
        folder_out + "hitmap_advanced_jet.png", tools_image.hitmap2d_to_jet(hitmap_2d)
    )
    cv2.imwrite(
        folder_out + "hitmap_advanced_viridis.png",
...
def get_self_hits(
    ...
    for period in range(0, max_period):
        print(period)
        disp_v1, disp_v2, disp_h1, disp_h2 = (
            -tol,
            +tol,
            (pattern_width) * (1 + period) - tol,
            (pattern_width) * (1 + period) + tol,
        )

        coord1, coord2, quality = tools_alg_match.get_best_matches(
            image, image, disp_v1, disp_v2, disp_h1, disp_h2, window_size, step
        )
        ...
        (
            ...
            mask1[period],
            mask2[period],
        ) = tools_alg_match.reproject_matches(
            image, image, coord1, coord2, window_size, False, (128, 128, 128)
        )
        acc_color[:, :] += reproject1[:, :].astype(numpy.float32) * mask1[
            period
        ].reshape((mask1.shape[1], mask1.shape[2], 1))
        acc_color[:, :] += reproject2[:, :].astype(numpy.float32) * mask2[
            period
        ].reshape((mask1.shape[1], mask1.shape[2], 1))

        if folder_debug is not None:
            cv2.imwrite(
                folder_debug + "%d-r1_%02d.png" % (window_size, 1 + period), reproject1
            )
            cv2.imwrite(
                folder_debug + "%d-r2_%02d.png" % (window_size, 1 + period), reproject2
            )
...
def get_best_pattern(images, weights, folder_debug=None):
    ...
    for i in range(0, images.shape[0] - 1):
        ...
        acc_color = numpy.zeros(
            (images.shape[1], images.shape[2], 3), dtype=numpy.float64
        )
        ...
        for j in range(i + 1, images.shape[0]):
            ...
            im1, im2, q = tools_calibrate.align_two_images_translation(
                images[j],
                images[i],
                detector="SIFT",
                matchtype="xxx",
...
def E2E_detect_patterns(
    ...
    if os.path.isfile(folder_out + "hitmap.png"):
        ...
        tools_IO.remove_files(folder_out)
        cv2.imwrite(
            folder_out + "%d-hitmap_00.png" % (window_size),
    ...
    coord = tools_alg_grid_templates.find_local_peaks(
        hitmap, int(pattern_height * peak_factor), int(pattern_width * peak_factor)
    )
    for i in range(0, coord.shape[0]):
        image_gray = tools_draw_numpy.draw_circle(
            image_gray, coord[i, 0], coord[i, 1], 5, (0, 64, 255)
        )
    ...
    hitmap = tools_alg_match.calc_hit_field_basic(
        image, pattern, rotation_tol=10, rotation_step=0.5
    )
    cv2.imwrite(
        folder_out + "%d-hitmap_01_jet.png" % (window_size),
        tools_image.hitmap2d_to_jet(hitmap),
    )
    cv2.imwrite(
        folder_out + "%d-hitmap_01_vrd.png" % (window_size),
    ...
    for i in range(0, coord.shape[0]):
        value = int(
            85 * (hitmap[coord[i, 0], coord[i, 1]] - min_value) / (255 - min_value)
        )
        image_gray = tools_draw_numpy.draw_circle(
            image_gray,
            coord[i, 0],
    ...
    image_gray_and_miss = tools_image.put_layer_on_image(
        image_gray, image_miss, background_color=(0, 0, 0)
    )
    image_gray_and_miss = tools_image.blend_avg(
        image_gray, image_gray_and_miss, weight=0.5
    )
