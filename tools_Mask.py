...
class Mask(object):
    ...
    def init_from_image(self, image_mask):
        ...
        self.corners_xy = (
            ...
            + numpy.array(
                numpy.unravel_index(
                    xx, (image_corners.shape[0], image_corners.shape[1])
                )
            ).T
        )
        ...
        for c in range(image_mask.shape[1]):
            ...
            if len(idx) > 0:
                ids = self.ids[idx]
                self.vert_n[idx] = numpy.array(
                    [(ids[i + 1], ids[i]) for i in range(0, len(ids), 2)]
                ).flatten()
    ...
    def save_debug(self):
        ...
        image_temp = tools_draw_numpy.draw_points(
            image_temp, 1 + 2 * self.corners_xy[self.signs > 0], color=(0, 32, 255), w=0
        )
        image_temp = tools_draw_numpy.draw_points(
            image_temp,
        ...
        for j in range(myL, image2d.shape[0] - myR):
            for i in range(mxL, image2d.shape[1] - mxR):
                ...
                s = (
                    ...
                    + self.NN * 255
                    - (
                        +image_integral[j + myR][i + mxR]
                        - image_integral[j + myR][i - mxL]
                        - image_integral[j - myL][i + mxR]
                        + image_integral[j - myL][i - mxL]
                    )
                )
