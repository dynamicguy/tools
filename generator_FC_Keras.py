...
class generator_FC_Keras(object):
    ...
    def learn(self, features, flat_images, grayscale):
        ...
        hist = self.model.fit(
            features[idx_train].astype(numpy.float32),
            flat_images[idx_train].astype(numpy.float32),
            nb_epoch=self.epochs,
            verbose=self.verbose,
            validation_data=(
                features[idx_valid].astype(numpy.float32),
                flat_images[idx_valid].astype(numpy.float32),
            ),
