...
class ELSD(object):
    ...
    def extract_ellipses(self, image, min_size=50):
        ...
        if os.path.isfile(self.folder_out + temp_eli + ".txt"):
            ellipses = tools_IO.load_mat(
                self.folder_out + temp_eli + ".txt", dtype=numpy.float, delim=" "
            )
