...
class ObjLoader:
    ...
    def load_mesh(self, filename_obj, do_autoscale=True):
        ...
        for line in open(filename_obj, "r"):
            ...
            if values[0] == "vt":
                self.coord_texture.append([float(v) for v in values[1:3]])
            if values[0] == "vn":
                self.coord_norm.append([float(v) for v in values[1:4]])
        ...
        if len(self.coord_texture) == 0:
            self.coord_texture.append([0, 0])
    ...
    def convert(self, filename_in, filename_out, do_normalize=True, cutoff=None):
        ...
        for line in lines:
            ...
            values = numpy.array(values)
            if values[0] == "v":
                coord_vert.append([float(v) for v in values[1:4]])
        ...
        self.export_mesh(
            filename_out, coord_vert, idx_vertex, do_transform=False, cutoff=cutoff
        )
    ...
    def export_mesh(
        ...
        for x in X:
            f_handle.write("v %1.2f %1.2f %1.2f\n" % (x[0], x[1], x[2]))

        if coord_texture is None:
            f_handle.write("vt 0 0\n")
        else:
            for t in coord_texture:
                f_handle.write("vt %1.2f %1.2f\n" % (t[0], t[1]))
        ...
        for n, t in enumerate(del_triangles):
            ...
            tx = (1, 1, 1)
            if coord_texture is not None:
                tx = (t[0] + 1, t[1] + 1, t[2] + 1)
