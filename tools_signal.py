...
def get_phases(signal, framestamps, frame_start, frame_stop, num_phases):
    ...
    signal2 = (
        signal
        - signal[frame_start - framestamps[0] : frame_stop - framestamps[0]].mean()
    )

    idx_down = zerro_crossings_down(signal2)
    idx_up = zerro_crossings_up(signal2)

    idx_down = numpy.delete(
        idx_down, numpy.where(idx_down < frame_start - framestamps[0])
    )
    idx_down = numpy.delete(
        idx_down, numpy.where(idx_down > frame_stop - framestamps[0])
    )
    ...
    for i in numpy.arange(0, idx_down.shape[0] - 1, 1):
        ...
        phase[start:stop] = numpy.linspace(0, int(num_phases / 2), stop - start).astype(
            int
        )
        start, stop = idx_up[i], idx_down[i + 1]
        phase[start:stop] = numpy.linspace(
            int(num_phases / 2), num_phases - 1, stop - start
        ).astype(int)
...
def approximate_periodical0(signal, phase, timerange=None):
    ...
    F = numpy.poly1d(
        numpy.polyfit(
            phase[numpy.where(phase != 0)], signal[numpy.where(phase != 0)], N
        )
    )
