...
class tools_ML(object):
    ...
    # ---------------------------------------------------------------------------------------------------------------------
    def prepare_arrays_from_feature_files(
        self,
        path_input,
        ...
        x = tools_IO.load_mat(
            path_input + ("%s%s" % (patterns[0], feature_mask)),
            numpy.chararray,
            delim="\t",
        )
        ...
        for i in range(0, patterns.shape[0]):
            x = tools_IO.load_mat(
                path_input + ("%s%s" % (patterns[i], feature_mask)),
                numpy.chararray,
                delim="\t",
            )
            if has_header:
    ...
    # ---------------------------------------------------------------------------------------------------------------------
    def prepare_tensors_from_image_folders(
        self,
        path_input,
        patterns,
        mask="*.png",
        ...
        # Y, all_filenames=[0],['-']
        for i in range(0, patterns.shape[0]):
            images, labels, filenames = tools_IO.load_aligned_images_from_folder(
                path_input + patterns[i] + "/",
                i,
                mask=mask,
                limit=limit,
                resize_W=resize_W,
    ...
    def score_feature_file(
        ...
        header, Y_test, X_test = self.preprocess_header(
            data_test, has_header=has_header, has_labels_first_col=has_labels_first_col
        )
    ...
    def train_test(self, X, Y, idx_train, idx_test, path_output=None):
        ...
        (
            ...
            labels_test_prob,
            challangers_test,
            challangers_test_prob,
        ) = self.predict(numpy.unique(Y), X[idx_test])
        ...
        (
            ...
            labels_train_prob,
            challangers_train,
            challangers_train_prob,
        ) = self.predict(numpy.unique(Y), X[idx_train])
    ...
    def learn_on_pos_neg_files(
        ...
        X_train_pos = (
            tools_IO.load_mat_pd(file_train_pos, numpy.chararray, delimeter)
        ).astype(numpy.str)
        X_train_neg = (
            tools_IO.load_mat_pd(file_train_neg, numpy.chararray, delimeter)
        ).astype(numpy.str)
        ...
        Y_train = numpy.hstack(
            (numpy.full(X_train_pos.shape[0], +1), numpy.full(X_train_neg.shape[0], -1))
        )
    ...
    def generate_data_grid(
        ...
        for s1 in range(0, step):
            for s2 in range(0, step):
                data_grid[s1 + (step - 1 - s2) * step, 0] = minx + s1 * (
                    maxx - minx
                ) / (step - 1)
                data_grid[s1 + (step - 1 - s2) * step, 1] = miny + s2 * (
                    maxy - miny
                ) / (step - 1)

        tools_IO.save_data_to_feature_file_float(
            filename_data_grid, data_grid, target_grid
        )
    ...
    def stage_train_stats(
        ...
        predictions = numpy.array(
            [patterns[labels_fact], patterns[labels_pred], labels_prob]
        ).T
        tools_IO.save_mat(
            predictions, path_output + self.classifier.name + "_predictions.txt"
        )
        tools_IO.print_accuracy(labels_fact, labels_pred, patterns)
        tools_IO.print_accuracy(
            labels_fact,
            labels_pred,
            patterns,
            filename=path_output + self.classifier.name + "_confusion_mat.txt",
        )
        tools_IO.print_top_fails(
            labels_fact,
            labels_pred,
            patterns,
            filename=path_output + self.classifier.name + "_errors.txt",
        )
        tools_IO.print_reject_rate(
            labels_fact,
            labels_pred,
            labels_prob,
            filename=path_output + self.classifier.name + "_accuracy.txt",
        )
    ...
    def E2E_images(
        ...
        (X, Y, filenames) = self.prepare_tensors_from_image_folders(
            path_input,
            patterns,
            mask=mask,
        ...
        idx_train = numpy.sort(
            numpy.random.choice(X.shape[0], int(X.shape[0] / 2), replace=False)
        )
    ...
    def E2E_features(
        ...
        if has_header:
            header = tools_IO.load_mat(
                path_input + ("%s%s" % (patterns[0], mask)), numpy.chararray, delim="\t"
            )
            header = header[0]
        else:
            header = None
    ...
    # ---------------------------------------------------------------------------------------------------------------------
    def E2E_features_2_classes_dim_2(
        self,
        folder_out,
        filename_data_pos,
        filename_data_neg,
        ...
        self.generate_data_grid(
            filename_data_pos, filename_data_neg, filename_data_grid
        )

        self.learn_on_pos_neg_files(
            filename_data_pos,
            filename_data_neg,
            delimeter="\t",
        ...
        tpr, fpr, auc = tools_IO.get_roc_data_from_scores_file_v2(
            filename_scrs_pos, filename_scrs_neg, delim="\t"
        )
        ...
        tools_IO.plot_2D_scores(
            ...
            th,
            noice_needed=1,
            caption=self.classifier.name + " %1.2f" % auc,
        )
        tools_IO.display_distributions(
            plt.subplot(1, 3, 2), fig, filename_scrs_pos, filename_scrs_neg, delim="\t"
        )
        tools_IO.display_roc_curve_from_descriptions(
            plt.subplot(1, 3, 3), fig, filename_scrs_pos, filename_scrs_neg, delim="\t"
        )
        plt.tight_layout()
    ...
    def check_corr(self, X, Y, header):
        ...
        C = numpy.zeros(N)
        for i in range(N):
            C[i] = math.fabs(numpy.corrcoef(X[:, i], Y)[0, 1])
        idx = numpy.argsort(-C)
    ...
    # ---------------------------------------------------------------------------------------------------------------------
    def plot_results_pos_neg(
        self, x_pos, x_neg, header, filename_scrs_pos, filename_scrs_neg, filename_out
    ):
        ...
        fig = plt.figure(figsize=(12, 6))
        fig.subplots_adjust(hspace=0.01)
        tools_plot.display_roc_curve_from_descriptions(
            plt.subplot(2, 2, 3), fig, filename_scrs_pos, filename_scrs_neg, delim="\t"
        )
        tools_plot.display_distributions(
            plt.subplot(2, 2, 2), fig, filename_scrs_pos, filename_scrs_neg, delim="\t"
        )
    ...
    # ---------------------------------------------------------------------------------------------------------------------
    def plot_results_train_test(
        self,
        filename_scores_train,
        filename_scores_test,
        filename_out,
        has_header=False,
    ):
        ...
        tpr, fpr, auc = tools_IO.get_roc_data_from_scores_file(
            filename_scores_train, has_header=has_header
        )
        tools_plot.plot_tp_fp(
            plt.subplot(1, 2, 1), fig, tpr, fpr, auc, caption="Train", filename_out=None
        )

        tpr, fpr, auc = tools_IO.get_roc_data_from_scores_file(
            filename_scores_test, has_header=has_header
        )
        tools_plot.plot_tp_fp(
            plt.subplot(1, 2, 2), fig, tpr, fpr, auc, caption="Test", filename_out=None
        )
    ...
    # ---------------------------------------------------------------------------------------------------------------------
    def E2E_features_2_classes_multi_dim(
        self,
        folder_out,
        filename_data_pos,
        filename_data_neg,
        ...
        header, first_col, x_pos = self.preprocess_header(
            data_pos, has_header, has_labels_first_col
        )
        header, first_col, x_neg = self.preprocess_header(
            data_neg, has_header, has_labels_first_col
        )

        numpy.random.seed(125)
        idx_pos_train = numpy.random.choice(
            x_pos.shape[0], int(x_pos.shape[0] / 2), replace=False
        )
        idx_neg_train = numpy.random.choice(
            x_neg.shape[0], int(x_neg.shape[0] / 2), replace=False
        )
        ...
        self.plot_results_pos_neg(
            x_pos,
            x_neg,
            header,
            filename_scrs_pos,
            filename_scrs_neg,
            folder_out + "fig_roc.png",
        )

        return  # tpr, fpr, auc

    # ---------------------------------------------------------------------------------------------------------------------
    def E2E_features_2_classes_multi_dim_train_test(
        self,
        folder_out,
        filename_train,
        filename_test,
        ...
        idx_pos_train, idx_neg_train, idx_pos_test, idx_neg_test = (
            numpy.array(idx_pos_train),
            numpy.array(idx_pos_train),
            numpy.array(idx_pos_test),
            numpy.array(idx_neg_test),
        )
        ...
        filename_scrs_train = (
            folder_out + "scores_train_" + self.classifier.name + ".txt"
        )
        ...
        self.draw_GT_pred(
            filename_scrs_train, folder_out + "GT_train.png", th, has_header=True
        )
        self.draw_GT_pred(
            filename_scrs_test, folder_out + "GT_test.png", th, has_header=True
        )
