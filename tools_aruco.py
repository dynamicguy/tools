...
def detect_marker_and_draw_axes(frame, marker_length, camera_matrix, dist):
    corners, ids, rejectedImgPoints = aruco.detectMarkers(
        cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY), aruco.Dictionary_get(aruco.DICT_6X6_50)
    )
    ...
    if len(corners) > 0:
        ...
        rvec, tvec, _ = aruco.estimatePoseSingleMarkers(
            corners[0], marker_length, camera_matrix, dist
        )
        # aruco.drawAxis(res, camera_matrix, dist, rvec[0], tvec[0], marker_length / 2)
        res = tools_render_CV.draw_axis(
            res, camera_matrix, dist, rvec[0], tvec[0], marker_length / 2
        )
...
def draw_cube(image_texture=None, pos_x=0, pos_y=0, pos_z=0, size=1):
    ...
    if image_texture != None:
        ...
        glTexImage2D(
            ...
            0,
            GL_RGB,
            image_texture.shape[1],
            image_texture.shape[0],
            0,
            GL_BGR,
            GL_UNSIGNED_BYTE,
            image_texture.tobytes(),
        )

    glBegin(GL_QUADS)

    glTexCoord2f(0.0, 0.0)
    glVertex3f(pos_x - size / 2, pos_y - size / 2, pos_z + size / 2)
