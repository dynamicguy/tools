...
class Ellipse_Processor(object):
    ...
    # ----------------------------------------------------------------------------------------------------------------
    def draw_segments(
        self, image, segments, color=(255, 255, 255), w=4, put_text=False
    ):
        ...
        for id, segment in enumerate(segments):
            ...
            if segment is None:
                continue
            if numpy.any(numpy.isnan(segment)):
                continue
            ...
            if put_text:
                ...
                cv2.putText(
                    result,
                    "{0}".format(id),
                    (min(W - 10, max(10, x)), min(H - 5, max(10, y))),
    ...
    def generate_ellipse_points(
        self, center_xy=(20, 10), Rxy=(5.0, 10.0), rotation_angle_deg=0, N=10
    ):
    ...
    def get_bbox_ellipse(self, ellipse):
        ...
        points = numpy.array(
            [[-Rxy[0], -Rxy[1]], [+Rxy[0], +Rxy[1]]], dtype=numpy.float32
        )
    ...
    def is_good_ellipse(self, ellipse):
        ...
        if numpy.any(numpy.isnan(ellipse[0])):
            return False
        if numpy.any(numpy.isnan(ellipse[1])):
            return False

        axes = (int(ellipse[1][0] / 2), int(ellipse[1][1] / 2))
        rotation_angle = ellipse[2]

        if axes[0] < 20 or axes[0] > 150:
            return False
        if axes[1] < 50 or axes[1] > 600:
            return False
        if axes[1] / axes[0] < 4.00:
            return False
        if axes[1] / axes[0] > 6.00:
            return False
        if not (
            (90 - 4 <= rotation_angle <= 90 + 4)
            or (270 - 4 <= rotation_angle <= 270 + 4)
        ):
            return False
    ...
    def estimate_ellipse(self, segments, s1, s2=None):
        ...
        if not self.is_good_ellipse(ellipse):
            return None, []
        ...
        is_match = numpy.array(
            [self.match_segment_ellipse(segment, ellipse_mask) for segment in segments]
        )
    ...
    # ----------------------------------------------------------------------------------------------------------------
    def preprocess_for_ellipse(
        self, segments, line_upper_bound=None, line_midfield=None
    ):
        filtered = []
        ...
        for segment in segments:
            ...
            # bottom check
            if box[1] + box[3] > self.H - 50:
                continue

            # top check
            if th_row is not None and box[1] < th_row:
                continue

            # left-right check
            if line_midfield is not None:
                if box[0] > line_midfield[0] + self.W / 4:
                    continue
                if box[0] + box[2] < line_midfield[0] - self.W / 4:
                    continue
            else:
                if angle is not None and angle < 90 and box[0] > 1 * self.W / 3:
                    continue
                if angle is not None and angle > 90 and box[0] < 2 * self.W / 3:
                    continue

            if line_midfield is not None:
                if (
                    tools_render_CV.distance_point_to_line(line_midfield, segment[0])
                    < 10
                    and tools_render_CV.distance_point_to_line(
                        line_midfield, segment[-1]
                    )
                    < 10
                ):
                    continue
    ...
    # ----------------------------------------------------------------------------------------------------------------
    def segments_to_ellipse(
        self, image, line_upper_bound, line_midfield, base_name=None, do_debug=False
    ):

        X, success = tools_IO.load_if_exists(
            self.folder_cache, base_name, "_ellipse.dat"
        )
        ...
        segments_double = self.Ske.keep_double_segments(
            segments_all, line_upper_bound, base_name
        )
        if line_upper_bound is not None:
            segments = self.preprocess_for_ellipse(
                segments_all, line_upper_bound, line_midfield
            )
        ...
        for s1 in range(len(segments) - 1):
            ...
            for s2 in range(s1 + 1, len(segments)):
                if processed[s1, s2] == 1:
                    continue
                processed[s1, s2] = 1

                bbox2 = numpy.array(cv2.boundingRect(segments[s2]))
                bbox2[2:] += bbox2[:2]
                if self.iou(bbox1, bbox2) > 0.2:
                    continue
                ...
                if do_debug and self.folder_out is not None:
                    ...
                    image_debug = self.draw_segments(
                        image_debug, segments, color=(0, 0, 128), w=8
                    )
                    image_debug = self.draw_segments(
                        image_debug, [segments[s1]], color=(90, 0, 255), w=8
                    )
                    image_debug = self.draw_segments(
                        image_debug, [segments[s2]], color=(0, 90, 255), w=8
                    )
                    ...
                    cv2.imwrite(
                        self.folder_out + base_name + "ellips_%03d_%03d.png" % (s1, s2),
                        image_debug,
                    )
