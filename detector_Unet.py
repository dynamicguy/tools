...
class detector_Unet(object):
    ...
    def do_stitch(self, img_h, img_w, patches, BORDER1, BORDER, PATCH_SIZE, color=True):
        ...
        for y in range(0, img_h, OUT_PATCH_SIZE):
            for x in range(0, img_w, OUT_PATCH_SIZE):
                patch = patches[idx, :, :]
                patch = patch[
                    BORDER : BORDER + OUT_PATCH_SIZE, BORDER : BORDER + OUT_PATCH_SIZE
                ]
    ...
    def fit(self, X_train, Y_train):
        ...
        checkpointer = ModelCheckpoint(
            self.folder_out + "chk.h5", verbose=1, save_best_only=True
        )
    ...
    def predict(self, image, threshold_prob=0.15):
        ...
        preds_arr[preds_arr <= threshold_prob] = 0.0
        preds_arr_t = preds_arr * 255
        pred_img = 255 * self.do_stitch(
            image.shape[0], image.shape[1], preds_arr_t, 8, 12, 256
        )
