...
# ---------------------------------------------------------------------------------------------------------------------
def folder_to_animated_gif_ffmpeg(
    path_input,
    path_out,
    filename_out,
    mask=".png",
    ...
    command_make_palette = 'ffmpeg -i %s%s%s -vf "palettegen" -y palette.png' % (
        prefix,
        "%06d",
        mask,
    )
    command_make_gif = (
        'ffmpeg -framerate %d -i %s%s%s -lavfi "scale=%d:%d:dither=none" -i palette.png %s -y'
        % (framerate, prefix, "%06d", mask, resize_W, resize_H, filename_out)
    )
...
# ---------------------------------------------------------------------------------------------------------------------
def folders_to_animated_gif_ffmpeg(
    path_input, path_out, mask=".png", framerate=10, resize_H=64, resize_W=64
):
    ...
    for subfolder in folderlist:
        ...
        folder_to_animated_gif_ffmpeg(
            path_input=path_input + subfolder + "/",
            path_out=path_out,
...
# ---------------------------------------------------------------------------------------------------------------------
def folder_to_animated_gif_imageio(
    path_input,
    filename_out,
    mask="*.png",
    framerate=10,
...
def folder_to_video(
    ...
    for b, filename in enumerate(fileslist):
        ...
        if resize_W is not None and resize_H is not None:
            image = cv2.resize(
                image, (resize_W, resize_H), interpolation=cv2.INTER_CUBIC
            )
        out.write(image)
...
def crop_images_in_folder(
    path_input, path_output, top, left, bottom, right, mask="*.jpg"
):
...
# ---------------------------------------------------------------------------------------------------------------------
def merge_images_in_folders(
    path_input1, path_input2, path_output, mask="*.png,*.jpg", rotate_first=False
):
    tools_IO.remove_files(path_output, create=True)
...
def merge_all_folders(
    list_of_folders, folder_out, target_W, target_H, filename_watermark=None
):
    ...
    for folder_in in list_of_folders:
        ...
        for filename_in in tools_IO.get_filenames(folder_in, "*.jpg,*.png"):
            base_name, ext = (
                filename_in.split("/")[-1].split(".")[0],
                filename_in.split("/")[-1].split(".")[1],
            )
            image = cv2.imread(folder_in + filename_in)
            image = tools_image.smart_resize(
                image, target_H - 2 * pad, target_W - 2 * pad, bg_color=bg_color
            )
