...
class Lines(object):
    ...
    def read_from_file(self, filename_in, dict_filenames):
        ...
        for each in data:
            ...
            if key in dict_filenames:
                ...
                self.xyxy.append(
                    [int(each[0]), int(each[1]), int(each[2]), int(each[3])]
                )
    ...
    def append_to_file(self, filename_out, imagenames, lines, IDs):
        ...
        for imagename, line, id in zip(imagenames, lines, IDs):
            if numpy.any(numpy.isnan(line)):
                continue
            tools_IO.save_raw_vec(
                [imagename, int(line[0]), int(line[1]), int(line[2]), int(line[3]), id],
                filename_out,
                mode=(os.O_RDWR | os.O_APPEND),
                fmt="%s",
                delim=" ",
    ...
    def add(self, filename_ID, xyxy, classID, do_standartize=True):
        self.filename_IDs = numpy.insert(
            self.filename_IDs, len(self.filename_IDs), filename_ID
        )
...
class Homographies(object):
    ...
    def append(self, filename_out, imagename, homography):
        ...
        tools_IO.save_raw_vec(
            xxx, filename_out, mode=(os.O_RDWR | os.O_APPEND), fmt="%s", delim=" "
        )
        return
    ...
    def read_from_file(self, filename_in, dict_filenames):
        ...
        for key in dict_filenames:
            if int(dict_filenames[key]) > last:
                continue
            ...
            if len(i) == 1:
                self.H.append(
                    numpy.array(data[i, 1:], dtype=numpy.float).reshape((3, 3))
                )
            else:
...
class Balls(object):
    ...
    def save_to_file(self, filename_out, filenames):
        ...
        for filename_ID, xyxy, cnf in zip(
            self.filename_IDs, self.xyxy, self.confidence
        ):
            if numpy.any(numpy.isnan(xyxy)):
                continue
            tools_IO.save_raw_vec(
                [
                    filenames[filename_ID],
    ...
    def transform(self, h_3x3):
        if self.xyxy is not None and len(self.xyxy) > 0:
            self.xyxy = tools_pr_geom.perspective_transform(
                numpy.array(self.xyxy).reshape((-1, 1, 2)).astype(numpy.float32), h_3x3
            ).reshape((-1, 4))
        return
...
class Players(object):
    ...
    def add(self, filename_ID, xyxy, classID):
        self.filename_IDs = numpy.insert(
            self.filename_IDs, len(self.filename_IDs), int(filename_ID)
        ).astype(numpy.int)
        xyxy = self.standartize(xyxy)
        if len(self.xyxy) > 0:
            self.xyxy = numpy.vstack(
...
class Polygons(object):
    ...
    def read_from_file(self, filename_in, dict_filenames):
        ...
        if not os.path.isfile(filename_in):
            return
        # data = tools_IO.load_mat_pd(filename_in, delim=' ', dtype=numpy.chararray)
        data = tools_IO.load_mat_var_size(
            filename_in, dtype=numpy.chararray, delim=" "
        )[1:]
