...
class tools_ML_enhanced(object):
    ...
    def df_to_XY(self, df, idx_target):
        ...
        are_categoirical = numpy.array(
            [cc in ["object", "category", "bool"] for cc in col_types]
        )
    ...
    def get_datagrid_for_2d(self, X, N=10):
        ...
        xx, yy = numpy.meshgrid(
            numpy.linspace(minx, maxx, num=N), numpy.linspace(miny, maxy, num=N)
        )
    ...
    def plot_ROC(self, df, idx_target, filename_out):
        ...
        accuracy = self.get_accuracy(
            tpr_train,
            fpr_train,
            nPos=numpy.count_nonzero(Y > 0),
            nNeg=numpy.count_nonzero(Y <= 0),
        )
        plt.clf()
        tools_plot.plot_tp_fp(
            plt, None, tpr_train, fpr_train, auc_train, filename_out=filename_out
        )
        return accuracy
    ...
    def plot_density(self, df, idx_target, filename_out):
        ...
        grid_confidence = self.predict_X(numpy.c_[xx.flatten(), yy.flatten()]).reshape(
            (N, N)
        )
        ...
        plt.clf()
        plt.contourf(xx, yy, grid_confidence, cmap=cm.coolwarm, alpha=0.8)
    ...
    def E2E_train_test_df(self, df, idx_target, idx_columns=None):
        ...
        acc_train = self.plot_ROC(
            df_train, idx_target, self.folder_out + "ROC_train.png"
        )
    ...
    def predict_X(self, X):
        scores = numpy.array(
            [(100 * self.classifier.predict(x)[:, 1]).astype(int) for x in X]
        )
        return scores
