...
class detector_YOLO3(object):
    ...
    def init_tiny(self, default_weights_h5, num_classes):
        ...
        (
            ...
            self.obj_threshold,
            self.nms_threshold,
        ) = tools_YOLO.init_default_metadata_tiny(num_classes)
        ...
        (
            ...
            self.scores,
            self.classes,
        ) = detector_YOLO3_core.get_tensors_box_score_class(
            self.model.output,
            self.anchors,
            self.anchor_mask,
    ...
    def __load_model(self, model_weights_h5, filename_metadata, obj_threshold=None):
        ...
        (
            ...
            self.obj_threshold,
            self.nms_threshold,
        ) = tools_YOLO.load_metadata(filename_metadata)
    ...
    def process_image(self, image):
        ...
        image_resized = tools_image.smart_resize(
            image, self.input_image_size[0], self.input_image_size[1]
        )
    ...
    def process_file_debug(self, filename_in, folder_out):
        ...
        (
            ...
            u_scores,
            u_classes,
        ) = detector_YOLO3_core.get_tensors_box_score_class_unfilter(
            self.model.output,
            self.anchors,
            self.anchor_mask,
            len(self.class_names),
        ...
        for c in list(set(classes)):
            ...
            total_image = tools_YOLO.draw_classes_on_image(
                total_image,
                boxes_yxyx[idx],
                scores[idx],
                self.colors[c],
            ...
            cv2.imwrite(
                folder_out
                + "class_%02d-%s-p%02d.png"
                % (c, self.class_names[c], 100 * scores[idx].max()),
                temp_image,
            )
    ...
    def process_folder(
        ...
        print(
            "Processing: %s sec in total - %f per image"
            % (total_time, int(total_time) / len(local_filenames))
        )
        return

    # ----------------------------------------------------------------------------------------------------------------------
    def process_folder_negatives(
        self,
        path_input,
        path_out,
        list_of_masks="*.png,*.jpg",
        ...
        for local_filename in local_filenames:
            ...
            if len(scores) > 0 and scores[0] >= confidence:
                filename_out = (
                    path_out + "%02d_" % int(100 * scores[0]) + local_filename
                )
                tools_YOLO.draw_and_save(
                    filename_out,
                    image,
    ...
    def get_bottleneck_features(self, filenames_list, dict_bottlenects):
        ...
        for filename in filenames_list:
            image_resized = tools_image.smart_resize(
                cv2.imread(filename), self.input_image_size[0], self.input_image_size[1]
            )
    ...
    def save_bottleneck_features(self, folder_out, filenames_list, dict_bottlenects):
        ...
        store0 = tools_HDF5.HDF5_store(
            filename=folder_out + "bottlenecks_0.hdf5",
            object_shape=bottlenecks[0][0].shape,
            dtype=numpy.float32,
        )
        store1 = tools_HDF5.HDF5_store(
            filename=folder_out + "bottlenecks_1.hdf5",
    ...
    def generate_features(
        self, path_input, path_output, limit=1000000, mask="*.png,*.jpg"
    ):
        ...
        patterns = numpy.sort(
            numpy.array(
                [
                    f.path[len(path_input) :]
                    for f in os.scandir(path_input)
                    if f.is_dir()
                ]
            )
        )
        ...
        for each in patterns:
            ...
            if not os.path.isfile(feature_filename):
                ...
                for b, local_filename in enumerate(local_filenames):
                    ...
                    image_resized = numpy.expand_dims(image_resized / 255.0, axis=0)
                    bottlenecks = Model(self.model.input, outputs).predict(
                        image_resized
                    )
                ...
                mat = numpy.zeros((features.shape[0], features.shape[1] + 1)).astype(
                    numpy.str
                )
                mat[:, 0] = filenames
                mat[:, 1:] = features
    ...
    def save_default_tiny_model_metadata(
        self,
        ...
        out_model = detector_YOLO3_core.yolo_body_tiny(
            Input(shape=(None, None, 3)), 3, num_classes
        )
    ...
    # ----------------------------------------------------------------------------------------------------------------------
    def save_default_full_model_metadata(
        self,
        default_model_weights,
        num_classes,
        filename_model_out,
        ...
        (
            ...
            obj_threshold,
            nms_threshold,
        ) = tools_YOLO.init_default_metadata_full(num_classes)
        tools_YOLO.save_metadata(
            filename_metadata_out,
            input_image_size,
            class_names,
    ...
    def construct_last_layers_placeholders(self, L, dict_last_layers, dict_bottlenects):
        ...
        placeholder_btlncs = [
            Input(
                shape=self.model.layers[L + i].output.shape[1:].as_list(),
                name="btlncs%d" % n,
            )
            for n, i in enumerate(dict_bottlenects.values())
        ]
        ...
        for key in dict_last_layers.keys():
            ...
            if numpy.min(values) >= 0:
                if len(values) == 1:
                    ...
                    xxx = []
                    for i in values:
                        xxx.append(placeholder_btlncs[i])
                ...
                if key < 0 and (key not in mid_layer):
                    ...
                    if numpy.max(values) < 0:
                        if len(values) == 1 and (values[0] in mid_layer):
                            ...
                            if (
                                numpy.min([1 * (each in mid_layer) for each in values])
                                == 1
                            ):
                                xxx = []
                                for i in values:
                                    xxx.append(mid_layer[i])
                                mid_layer[key] = Concatenate()(xxx)
                                flag = 1
    ...
    def __calc_loss(
        ...
        for i in range(bottlenecks[0].shape[0]):
            dct = {
                placeholder_btlncs[0]: numpy.array([bottlenecks[0][i]]),
            ...
            if nlyrs == 3:
                ...
                dct[placeholder_yolo_features[2]] = numpy.array(
                    numpy.array([targets[2][i]])
                )
    ...
    def __last_tiny_layers14(self):
        ...
        dict_last_layers[-15], dict_last_layers[-10], dict_last_layers[-25] = (
            [-16],
            [-11],
            [-26],
        )
        dict_last_layers[-16], dict_last_layers[-11], dict_last_layers[-26] = (
            [-17],
            [-12],
            [-27],
        )
        dict_last_layers[-17], dict_last_layers[-12], dict_last_layers[-27] = (
            [-18],
            [-13],
            [-28],
        )
        dict_last_layers[-18], dict_last_layers[-13], dict_last_layers[-28] = (
            [-19],
            [-14],
            [-29],
        )
        dict_last_layers[-19], dict_last_layers[-14], dict_last_layers[-29] = (
            [-20],
            [-15],
            [-30],
        )
    ...
    # ----------------------------------------------------------------------------------------------------------------------
    def __last_full_layers4(self):
        dict_last_layers, dict_bottlenects = {}, {}

        dict_last_layers[0] = [-3, -2, -1]
        dict_last_layers[-3], dict_last_layers[-2], dict_last_layers[-1] = (
            [-6],
            [-5],
            [-4],
        )
        dict_last_layers[-6], dict_last_layers[-5], dict_last_layers[-4] = (
            [-9],
            [-8],
            [-7],
        )
        dict_last_layers[-9], dict_last_layers[-8], dict_last_layers[-7] = [0], [1], [2]

        dict_bottlenects[0] = -12
        dict_bottlenects[1] = -11
        dict_bottlenects[2] = -10
        return dict_last_layers, dict_bottlenects

    # ----------------------------------------------------------------------------------------------------------------------
    def data_generator(
        self,
        list_store_bottlenecks,
        list_store_targets,
        ...
        while True:
            ...
            for b in range(batch_size):
                ...
                for i in range(len(list_store_bottlenecks)):
                    bottlenecks[i].append(list_store_bottlenecks[i].get(g))
                for i in range(len(list_store_targets)):
                    targets[i].append(list_store_targets[i].get(g))
                if is_train == True:
            ...
            for i in range(len(bottlenecks)):
                bottlenecks[i] = numpy.array(bottlenecks[i])
            for i in range(len(targets)):
                targets[i] = numpy.array(targets[i])
            yield [*bottlenecks, *targets], numpy.zeros((batch_size, 1))

    # ----------------------------------------------------------------------------------------------------------------------
    def __fit_loss_model(
        self, model_last_layers, placeholder_btlncs, bottlenecks, targets
    ):

        n_epochs = 100
        lambda_object = Lambda(
            function=detector_YOLO3_core.yolo_loss,
            output_shape=(1,),
            name="yolo_loss",
            arguments={
        ...
        placeholder_yolo_features = [
            Input(shape=(13, 13, 3, 5 + len(self.class_names))),
            Input(shape=(2 * 13, 2 * 13, 3, 5 + len(self.class_names))),
        ]
        if len(model_last_layers.output) == 3:
            placeholder_yolo_features.append(
                Input(shape=(52, 52, 3, 5 + len(self.class_names)))
            )

        tensor_loss = lambda_object(
            [*model_last_layers.output, *placeholder_yolo_features]
        )

        model_loss = Model(
            inputs=[*placeholder_btlncs, *placeholder_yolo_features],
            outputs=tensor_loss,
        )
        model_loss.compile(
        ...
        print("Learning ...")
        model_loss.fit(
            x=[*bottlenecks, *targets],
            y=numpy.zeros((targets[0].shape[0], 1)),
            validation_split=0.3,
            verbose=2,
            epochs=n_epochs,
    ...
    def __fit_loss_model_gen(self, model_last_layers, placeholder_btlncs, folder_in):
        ...
        store_bottlenecks0 = tools_HDF5.HDF5_store(
            filename=folder_in + "bottlenecks_0.hdf5"
        )
        store_bottlenecks1 = tools_HDF5.HDF5_store(
            filename=folder_in + "bottlenecks_1.hdf5"
        )
        ...
        if use_generator == False:
            ...
            if len(self.anchor_mask) > 2:
                targets.append(store_target2.get())
                store_bottlenecks2 = tools_HDF5.HDF5_store(
                    filename=folder_in + "bottlenecks_2.hdf5"
                )
                bottlenecks.append(store_bottlenecks2.get())
            ...
            generator_train = self.data_generator(
                [store_bottlenecks0, store_bottlenecks1],
                [store_target0, store_target1],
                validation_split=0.3,
                batch_size=32,
                is_train=True,
            )
            generator_val = self.data_generator(
                [store_bottlenecks0, store_bottlenecks1],
                [store_target0, store_target1],
                validation_split=0.3,
                batch_size=32,
            ...
            model_loss.fit_generator(
                generator_train,
                validation_data=generator_val,
                epochs=n_epochs,
                steps_per_epoch=store_target0.size // 32,
                validation_steps=1,
                verbose=2,
                callbacks=[early_stopping_monitor],
            )
    ...
    def learn(
        ...
        list_filenames = sorted(
            set([foldername + line.split(" ")[0] for line in lines])
        )
        true_boxes = tools_YOLO.get_true_boxes(
            foldername, file_annotations, (416, 416), delim=" ", limit=limit
        )
        list_filenames = sorted(
            set([foldername + line.split(delim)[0] for line in lines])
        )
        true_boxes = tools_YOLO.get_true_boxes(
            foldername, file_annotations, (416, 416), delim=delim, limit=limit
        )
        detector_YOLO3_core.save_targets(
            folder_out,
            true_boxes,
            (416, 416),
            self.anchors,
        ...
        if len(true_boxes) > 6:
            self.anchors = tools_YOLO.annotation_boxes_to_ancors(
                true_boxes, len(self.anchors)
            )
            (
                self.boxes,
                self.scores,
        ...
        placeholder_btlncs, last_layers_outs = self.construct_last_layers_placeholders(
            len(self.model.layers), dict_last_layers, dict_bottlenects
        )

        self.save_bottleneck_features(folder_out, list_filenames, dict_bottlenects)

        model_last_layers = Model(
            inputs=[*placeholder_btlncs], outputs=[*last_layers_outs]
        )
        ...
        self.__load_model(
            folder_out + "A_model.h5", folder_out + "A_metadata.txt", self.obj_threshold
        )
