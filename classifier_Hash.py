...
class classifier_Hash(object):
    ...
    def predict_one(self, x):
        ...
        if key in self.dct_pos.keys():
            if key not in self.dct_neg:
                ...
                result = numpy.array(
                    [0, self.dct_pos[key] / (self.dct_pos[key] + self.dct_neg[key])]
                )
    ...
    def build_domains(self, filename_in):
        ...
        for c in range(1, C, 1):
            ...
            domain = {}
            feature = numpy.array(
                tools_IO.get_columns(filename_in, delim="\t", start=c, end=c + 1)
            ).flatten()
            cnt = 0
            for val in feature:
    ...
    def calc_counts_file(self, filename_in, has_header=False, delim="\t"):
        ...
        self.dct_pos, success1 = tools_IO.load_if_exists(
            self.folder_out + "dct_pos.dat"
        )
        self.dct_neg, success2 = tools_IO.load_if_exists(
            self.folder_out + "dct_neg.dat"
        )
