...
def get_hits(image_base, image_pattern, grid=0.1, path_debug=""):
    ...
    coordinates, chains_hor, chains_ver = detect_peaks_2d(
        image_base, image_pattern, hitmap_2d, grid=0.05, path_debug=path_debug
    )
...
def find_local_peaks(hitmap_2d, row_step, col_step):
    ...
    for row in range(0, R_steps):
        for col in range(0, C_steps):
            ...
            if not (
                x == 0 or x == field.shape[0] - 1 or y == 0 or y == field.shape[1] - 1
            ):
...
def find_local_peaks_greedy(hitmap_2d, row_step, col_step, min_value=150):
    ...
    misses = ndimage.uniform_filter(
        field2, (int(row_step * 0.75), int(col_step * 0.75)), mode="constant"
    )
    misses_rgb = numpy.zeros(
        (hitmap_2d.shape[0], hitmap_2d.shape[1], 3), dtype=numpy.uint8
    )
...
def filter_unique(coord, hitmap_2d, tol):
    ...
    for i in range(0, coord.shape[0] - 1):
        for j in range(i + 1, coord.shape[0]):
            ...
            if dst < tol:
                if (
                    hitmap_2d[coord[i, 0], coord[i, 1]]
                    > hitmap_2d[coord[j, 0], coord[j, 1]]
                ):
                    idx.append(j)
                else:
                    idx.append(i)
...
def are_hor_nbr(row1, col1, row2, col2, tolerance_angle, max_nbr_distance):
    if (
        abs(row1 - row2) <= tolerance_angle * abs(col1 - col2)
        and abs(col1 - col2) <= max_nbr_distance
    ):
        return 1
    return 0
...
def are_ver_nbr(row1, col1, row2, col2, tolerance_angle, max_nbr_distance):
    if (
        abs(col1 - col2) <= tolerance_angle * abs(row1 - row2)
        and abs(row1 - row2) <= max_nbr_distance
    ):
        return 1
    return 0
    ...
    for l in reversed(range(0, L - 1)):
        ...
        for ll in range(l + 1, L):
            if (
                Nbr[idx[l], idx[ll]] == 1
                and SR[ll] > maxS
                and interceсt_chains(coord, all_chains, idx[l], idx[ll]) == 0
            ):
                maxS = SR[ll]
                dirR[l] = ll
    ...
    while flag == 1:
        ...
        for i in range(0, len(chains_hor) - 1):
            for j in range(i + 1, len(chains_hor)):
                algn_hor[i, j] = are_chains_aligned(
                    chains_hor[i], chains_hor[j], chains_ver
                )
        ...
        for i in range(0, len(chains_ver) - 1):
            for j in range(i + 1, len(chains_ver)):
                algn_ver[i, j] = are_chains_aligned(
                    chains_ver[i], chains_ver[j], chains_hor
                )
        ...
        if (len(chains_hor_new) != len(chains_hor)) or (
            len(chains_ver_new) != len(chains_ver)
        ):
            flag = 1
...
def filter_grid(coord, hitmap_2d, tolerance_angle, max_nbr_distance):
    ...
    while any_wiped == True:
        ...
        chains_hor, chains_ver = get_chains_hor_vert(
            coord, hitmap_2d, tolerance_angle, max_nbr_distance
        )
...
def detect_peaks_2d(image_base, image_pattern, hitmap_2d, grid=0.1, path_debug=""):
    ...
    coord = filter_grid(
        coord, hitmap_2d, tolerance_angle=1.0 / 5.0, max_nbr_distance=row_step * 120
    )
    chains_hor, chains_ver = get_chains_hor_vert(
        coord, hitmap_2d, tolerance_angle=1.0 / 5.0, max_nbr_distance=row_step * 120
    )
...
def coordinates_to_images(image_base, coordinates, dR, dC):
    ...
    for i in range(0, coordinates.shape[0]):
        ...
        if (
            left > 0
            and top > 0
            and bottom < image_base.shape[0]
            and right < image_base.shape[1]
        ):
            cut = tools_image.crop_image(
                image_base, top, left, bottom, right, extrapolate_border=True
            )
            images.append(cut)
...
def filter_outliers(image_base, coordinates, pattern, th=1.0, path_debug=""):
    ...
    images = coordinates_to_images(
        image_base, coordinates, pattern.shape[0], pattern.shape[1]
    )
...
def estimate_pattern_position(filename_in, folder_out, pattern_width, pattern_height):
    ...
    image_base_scaled = cv2.resize(
        image_base, (int(image_base.shape[1] / scale), int(image_base.shape[0] / scale))
    )
    ...
    for shift_row in range(0, pattern_height):
        for shift_col in range(0, pattern_width):
            ...
            image_pattern_scaled = cv2.resize(
                image_pattern,
                (
                    int(image_pattern.shape[1] / scale),
                    int(image_pattern.shape[0] / scale),
                ),
            )

            hitmap_2d = tools_alg_match.calc_hit_field(
                image_base_scaled, image_pattern_scaled
            )
            hitmap_RGB_jet = tools_image.hitmap2d_to_jet(-hitmap_2d)

            e = entropy(hitmap_2d.flatten() / 255.0)

            toimage(hitmap_RGB_jet).save_model(
                folder_out + ("hit_%1.3f_%03d_%03d.bmp" % (e, shift_row, shift_col))
            )
            toimage(image_pattern_scaled).save_model(
                folder_out + ("pat_%03d_%03d.bmp" % (shift_row, shift_col))
            )
    return
...
def draw_hits(path, coordinates, chains_hor, chains_ver, hitmap_2d):
    ...
    for each in chains_hor:
        for i in range(0, len(each) - 1):
            hitmap_RGB_gre = tools_draw_numpy.draw_line(
                hitmap_RGB_gre,
                coordinates[each[i]][0],
                coordinates[each[i]][1],
                coordinates[each[i + 1]][0],
                coordinates[each[i + 1]][1],
                [128, 128, 0],
            )
    ...
    for i in range(0, coordinates.shape[0]):
        hitmap_RGB_gre = tools_draw_numpy.draw_circle(
            hitmap_RGB_gre, coordinates[i][0], coordinates[i][1], 4, [255, 32, 0]
        )
