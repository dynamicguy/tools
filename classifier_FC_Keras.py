...
class classifier_FC_Keras(object):
    ...
    def init_model_conv(self, input_shape, num_classes):
        ...
        self.model.add(
            Dense(
                numpy.prod(self.model.layers[-3].output.get_shape().as_list()[1:]),
                activation="relu",
            )
        )
    ...
    def learn(self, data_train, target_train):
        ...
        hist = self.model.fit(
            data_train[idx_train].astype(numpy.float32),
            target_train[idx_train],
            nb_epoch=self.epochs,
            verbose=self.verbose,
            validation_data=(data_train[idx_valid], target_train[idx_valid]),
    ...
    def images_to_features(self, images):
        ...
        if (
            images.shape[1] != self.model.input_shape[1]
            or images.shape[2] != self.model.input_shape[2]
        ):
            images[:] = cv2.resize(
                images[:], (self.model.input_shape[1], self.model.input_shape[2])
            )

        features = Model(
            inputs=self.model.input, outputs=self.model.layers[feature_layer].output
        ).predict(numpy.array(images))
        return features
    ...
    def generate_features(self, path_input, path_output, limit=1000000, mask="*.png"):
        ...
        for each in patterns:
            ...
            local_filenames = numpy.array(
                fnmatch.filter(listdir(path_input + each), mask)
            )[:limit]
            ...
            if not os.path.isfile(feature_filename):
                for i in range(0, local_filenames.shape[0]):
                    ...
                    image = cv2.resize(
                        image, (self.model.input_shape[1], self.model.input_shape[2])
                    )
                    feature = Model(
                        inputs=self.model.input,
                        outputs=self.model.layers[feature_layer].output,
                    ).predict(numpy.array([image]))
    ...
    def feature_to_layers(self, feature, path_output):
        ...
        invmodel = tools_CNN_view.add_de_conv_layer(
            invmodel, orig_layers[3], crop=1, input_shape=feature.shape
        )
        ...
        outputs = Model(
            inputs=invmodel.input, outputs=[layer.output for layer in invmodel.layers]
        ).predict(numpy.array([feature]))
        names = [layer.name for layer in invmodel.layers]

        feature_image = tools_CNN_view.tensor_gray_3D_to_image(feature)
        cv2.imwrite(
            path_output + "feature.png", feature_image * 255.0 / feature_image.max()
        )
