...
# ----------------------------------------------------------------------------------------------------------------------
def stack_frames(
    folder_in, filename_out, h=32, w=18, stride=1, basis=1000, columns=100, put_ID=False
):
...
def fill_empties(tensor):
    ...
    image_na = numpy.full(
        (tensor.shape[0], tensor.shape[1], 3), clr_bg, dtype=numpy.uint8
    )
    R = tensor.shape[0]
