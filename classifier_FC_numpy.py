...
class classifier_FC_numpy(object):
    ...
    def init_layers(self, nn_architecture, seed=99):
        ...
        for idx, layer in enumerate(nn_architecture):
            ...
            params_values["W" + str(layer_idx)] = (
                numpy.random.randn(layer_output_size, layer_input_size) * 0.1
            )
            params_values["b" + str(layer_idx)] = (
                numpy.random.randn(layer_output_size, 1) * 0.1
            )
    ...
    def single_layer_forward_propagation(
        self, A_prev, W_curr, b_curr, activation="relu"
    ):
    ...
    def full_forward_propagation(self, X):
        ...
        for idx, layer in enumerate(NN_ARCHITECTURE):
            ...
            A_curr, Z_curr = self.single_layer_forward_propagation(
    ...
    def get_cost_value(self, Y_hat, Y):
        ...
        cost = (
            ...
            / m
            * (
                numpy.dot(Y, numpy.log(Y_hat).T)
                + numpy.dot(1 - Y, numpy.log(1 - Y_hat).T)
            )
        )
    ...
    def single_layer_backward_propagation(
        self, dA_curr, W_curr, b_curr, Z_curr, A_prev, activation="relu"
    ):
    ...
    def full_backward_propagation(
        self, Y_hat, Y, memory, params_values, nn_architecture
    ):
        grads_values = {}
    ...
    def train(self, X, Y):
        ...
        for i in range(self.epochs):
            ...
            grads_values = self.full_backward_propagation(
                Y_hat, Y, cashe, self.params_values, NN_ARCHITECTURE
            )
            # updating model state
            self.params_values = self.update(
                self.params_values, grads_values, NN_ARCHITECTURE, self.learning_rate
            )

            if (i % 1000 == 0) and self.verbose:
                print(
                    "Iteration: {:} - cost: {:.5f} - accuracy: {:.5f}".format(
                        i, cost, accuracy
                    )
                )
