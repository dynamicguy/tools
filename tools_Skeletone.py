...
class Skelenonizer(object):
    ...
    def binarize(self, image):
        ...
        binarized = cv2.adaptiveThreshold(
            gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 27, 0
        )
    ...
    def sraighten_segments(self, segments, min_len=10):
        ...
        for i, s in enumerate(segments):
            ...
            for each in straighten:
                result.append(each)
        return result

    # ----------------------------------------------------------------------------------------------------------------
    def sraighten_segment(self, segment, min_len=10, do_debug=True):
        ...
        if (
            numpy.all(dx_DL_good)
            or numpy.all(dx_DR_good)
            or numpy.all(dy_RD_good)
            or numpy.all(dy_RU_good)
        ):
            return [segment]
        ...
        if do_debug:
            cv2.imwrite(
                self.folder_out + "DL_bin.png", 255 * dx_DL_good.reshape((1, -1))
            )
            cv2.imwrite(
                self.folder_out + "DR_bin.png", 255 * dx_DR_good.reshape((1, -1))
            )
            cv2.imwrite(
                self.folder_out + "RD_bin.png", 255 * dy_RD_good.reshape((1, -1))
            )
            cv2.imwrite(
                self.folder_out + "RU_bin.png", 255 * dy_RU_good.reshape((1, -1))
            )

            image = numpy.full((720, 1280, 3), 64, dtype=numpy.uint8)
            cv2.imwrite(
                self.folder_out + "DL.png",
                tools_draw_numpy.draw_points(
                    image, segment_DL, tools_draw_numpy.get_colors(len(segment_DL)), w=1
                ),
            )
            cv2.imwrite(
                self.folder_out + "DR.png",
                tools_draw_numpy.draw_points(
                    image, segment_DR, tools_draw_numpy.get_colors(len(segment_DR)), w=1
                ),
            )
            cv2.imwrite(
                self.folder_out + "RD.png",
                tools_draw_numpy.draw_points(
                    image, segment_RD, tools_draw_numpy.get_colors(len(segment_RD)), w=1
                ),
            )
            cv2.imwrite(
                self.folder_out + "RU.png",
                tools_draw_numpy.draw_points(
                    image, segment_RU, tools_draw_numpy.get_colors(len(segment_RU)), w=1
                ),
            )
    ...
    def interpolate_segment_by_line_slow(self, XY):
        ...
        if (X.max() - X.min()) > (Y.max() - Y.min()):
            ...
            line = numpy.array(
                [X_inter[0], Y_inter[0], X_inter[1], Y_inter[1]]
            ).flatten()
        else:
            reg.fit(Y, X)
    ...
    # ----------------------------------------------------------------------------------------------------------------------
    def keep_double_segments(
        self, segments, line_upper_bound, base_name=None, do_debug=False
    ):
        ...
        for l1 in range(len(lines) - 1):
            ...
            if numpy.any(numpy.isnan(line1)):
                continue
            for l2 in range(l1 + 1, len(lines)):
                ...
                if numpy.any(numpy.isnan(line2)):
                    continue
                if self.are_same_segments(segment1, segment2):
                    continue
                ...
                if numpy.any(numpy.isnan((d1, d2, d3, d4))):
                    continue
                if d1 > tol_max or d2 > tol_max:
                    continue
                if d3 > tol_max or d4 > tol_max:
                    continue
        ...
        if do_debug:
            ...
            image_segm = tools_draw_numpy.draw_segments(
                empty, segments, colors_all, w=1, put_text=True
            )
            ...
            image_segm = tools_draw_numpy.draw_segments(
                image_segm, segments, (90, 90, 90), w=1
            )
            segments[~idx_good] = None
            image_segm = tools_draw_numpy.draw_segments(
                image_segm, segments, (0, 0, 255), w=1
            )
    ...
    def has_common_range(self, segment1, segment2, line1, line2):
        ...
        check_x = (max_x2 <= min_x1 and max_x2 <= max_x1) or (
            min_x2 >= min_x1 and min_x2 >= max_x1
        )
        check_y = (max_y2 <= min_y1 and max_y2 <= max_y1) or (
            min_y2 >= min_y1 and min_y2 >= max_y1
        )
    ...
    def skelenonize_fast(self, binarized):
        ...
        lines = cv2.HoughLinesP(
            edges, 1, numpy.pi / 180, 40, int(binarized.shape[0] * 0.1)
        )
